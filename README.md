...
- A **sharp phase transition** at $\sigma^* = 0.40$ at PP-scenario parameters ($\varepsilon = 0.35$, $n_{agg} = 25$), validated against the agent-based simulation in §4.1 (emergent crossing at realized $\sigma \approx 0.387$, within 0.013 of the analytical threshold; bands decide against emergent monument stock and realized network buffering, so the match is a genuine dynamic-sufficiency result). The §4.2 phase-space diagram across the joint $(\sigma, \varepsilon)$ plane evaluates the analytical threshold line against the simulated regime structure across the parameter space.
- **Signaling-vs-cooperation ablation (§4.3)**: signal-conditional partner formation does not move the threshold relative to a model in which the same number of cooperative ties form by random pairing ($\sigma_{eff}^*$ shifts by 0.4%, well within replicate noise). The framework's discriminating empirical content lies on the spatial-flow dimension (§6.1), not the threshold-location dimension.
...
- **Type-site calibration (anchor, not test)**: model output scales to ~750,000 m³ of earthwork (~104 m³ per investment unit; PP-scenario mean 7,203 ± 3,276 units across 8 replicates) over the active interval. The scaling factor is fit to the archaeological target.
- **Distance-decay (fails as an unconditional ordering prediction; would not discriminate even if passed)**: Spearman ρ = −0.40 across the five tracked materials against source distance (+0.50 restricted to the three far-source materials); the ordering is recovered only with material-specific value and recovery corrections, and the framework explicitly does not claim distance decay distinguishes signaling from generic transport-cost or down-the-line exchange alternatives.
- **Per-material posterior predictive check (§6.1)**: copper consistent (model 106 ± 30 vs archaeological 155, predictive interval contains the observation); galena consistent (702 ± 188 vs 740, no correction invoked); steatite consistent on a vessel basis (480 ± 116 vs "several hundred vessels"); the near-source materials are the informative failure (novaculite overpredicted ~63×, crystal quartz ~8× by the cost-driven acquisition rule). Joint Mahalanobis $d^2 = 9{,}289$ rejects the no-correction joint reading; cross-material correlations $r = 0.89$-$0.99$ mean apparent independence of marginal checks is overstated.
- **Watson Brake (§6.3, consistency demonstration conditional on a fitted labor-scaling exponent)**: 9× volume overprediction under continuous-equilibrium operation closes to 0.91× under per-event labor scaling at $\alpha = 2.0$; regime switching accounts for the episodic tempo rather than the volume. Erasmus's (1965) experimental rate of 2.6 m³/person-day implies $\alpha \approx 1$ under additive labor accounting, at which the model still overpredicts ~2.8×; $\alpha = 2$ requires superadditive scaling beyond what Erasmus supports.
- **Cross-LMV screening (§6.4, sample-selection-bounded)**: under the corrected static-diversity rubric, all nine interior monument-building sites sit in the high-$\varepsilon$ band and the coastal Pearl River pair (Claiborne, Cedarland) sits lower, but by a small margin ($\varepsilon = 0.40$ vs 0.40-0.49; the weight-perturbation Monte Carlo keeps PP first in 86% of draws but keeps the coastal pair in the bottom two ranks in only 21%). The qualitative rubric correlates significantly with the Saucier-geomorphic GIS index ($\rho = 0.70$, $p = 0.016$) but not with the EPA Level IV index ($\rho = 0.12$); none of the static indices predicts monument scale ($\rho = +0.39$, $+0.24$, $-0.21$, all n.s.), confirming that 25-km terrestrial buffer ecological diversity is not the operative quantity (see §6.5 below).
- **Multi-drainage shortfall buffering (§6.5, descriptive)**: modern USGS gauge data identify three substantively independent shortfall regimes at Poverty Point's confluence (Macon Ridge: Bayou Maçon + Tensas at $r = 0.90$ functioning as one; Mississippi mainstem; Yazoo Basin), the strongest multi-regime buffering among LMV mound-building sites. The temporal phenology hierarchy (§6.7, Figure 15) converges on the same conclusion: PP integrates five independent resource-peak windows, more than any other LMV site. Modern-as-proxy concession explicit.
- **Cross-site magnitude (§6.6, not predicted by the framework)**: joint $M_g(\varepsilon, n_{agg})$ correlation across 11 LMV sites returns Spearman ρ = +0.85 to +0.92, but partial-correlation decomposition shows this is essentially the exogenous $n_{agg}$ ranking from the convergence-model literature; $\varepsilon$ contributes at most ±0.02 marginal correlation. The framework's $\varepsilon$ is a necessity-condition variable, not a magnitude predictor.
...
├── data/
│   ├── site_coordinates.csv        # Site coordinates (lat/lon and UTM), generated from data/sites/late_archaic_sites.csv (canonical)
...
│   ├── calibration_replicates/     # Figure 12 replicate-spread data
...
The critical threshold $\sigma^*$ where aggregation becomes adaptive is solved numerically with Brent's (1973) method on the lambda-sigma feedback loop. At the PP-scenario operating point used throughout the §4 validation ($\varepsilon = 0.35$, $n_{agg} = 25$), $\sigma^* \approx 0.40$; at the Shannon-rubric $\varepsilon = 0.49$, $\sigma^* \approx 0.36$.
...
**Figure 1** uses the USGS digitization of Saucier (1994) Lower Mississippi River Valley geomorphology (ScienceBase item 59b7ddd6e4b08b1644df5cf6, doi:10.5066/F7N878QN, public domain). The figure distinguishes Holocene deposits (Mississippi alluvial valley, deltaic and chenier plains) from Pleistocene deposits (Wisconsin Stage valley trains, Prairie Complex including Macon Ridge, Deweyville Complex). Poverty Point's coordinates (NAD83, UTM Zone 15N: 649390 E / 3612125 N → 32.6366 N, −91.4074 W) fall on the Pleistocene `Pve` formation at the eastern margin of Macon Ridge. Site coordinates for all eleven Table 1 sites are drawn from `data/sites/late_archaic_sites.csv` (the canonical coordinate source, carrying the June 2026 coordinate corrections); `data/site_coordinates.csv` is a UTM-annotated export of the same coordinates.

**Figure 13** (and supplemental GIS analyses) use the EPA Office of Research and Development Level IV ecoregion classification (Omernik and Griffith 2014). Ecoregion shapefiles for Arkansas, Louisiana, and Mississippi are loaded from `data/ecoregions/{ar,la,ms}_eco_l4/`. The figure shows the 11 LMV mound-building sites with their 25 km foraging buffers overlaid on the Level IV polygon background. Watson Brake's buffer crosses the highest count of distinct Level IV polygons (7) of any LMV site, with Caney (6) and Frenchman's Bend (6) close behind; PP integrates 4. The 25-km terrestrial-buffer count does not predict observed monument scale. The framework's ε is operative through the *covariance-based* multi-drainage analysis of §6.5 (Figure 14) and the temporal-staggering analysis of §6.7 (Figure 15), not through static-buffer ecological diversity.
...
Generate the calibration replicates (Figure 12; ~3 hours for the underlying simulation):
```bash
make calibration
```
