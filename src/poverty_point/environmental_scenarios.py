...
paleoclimatic data. These scenarios create different levels of sigma (uncertainty)
through variation in shortfall frequency and magnitude.

Canonical sigma formula (from parameters.py):
    sigma = magnitude * sqrt(20 / frequency)

Based on calibration data from:
- Rapa Nui: High sigma environment (frequent/severe shortfalls)
- Rapa Iti: Low sigma environment (rare/mild shortfalls)
- Poverty Point: Moderate sigma with ecotone buffering
"""
...
from .parameters import calculate_sigma_from_shortfall
...
class ShortfallParams:
    ...
    These directly influence effective sigma in the simulation.
    """
...
class EnvironmentalScenario:
    ...
    different levels of environmental uncertainty (sigma).
    """
    name: str
    description: str
    ...
    # Expected sigma range (for validation)
    expected_sigma_range: Tuple[float, float]

    # Ecotone advantage (epsilon) at optimal aggregation location
...
def create_high_sigma_scenario(
    ...
    """
    Create a high-sigma scenario similar to Rapa Nui conditions.

    Canonical sigma = 0.60 * sqrt(20/6) = 1.095 -> clipped ~0.95
    ...
    return EnvironmentalScenario(
        ...
        expected_sigma_range=(0.70, 0.95),
...
def create_low_sigma_scenario(
    ...
    """
    Create a low-sigma scenario similar to Rapa Iti conditions.

    Canonical sigma = 0.30 * sqrt(20/18) = 0.316
    ...
    return EnvironmentalScenario(
        ...
        env_config=env_config,
        shortfall_params=shortfall_params,
        expected_sigma_range=(0.20, 0.40),
        expected_epsilon=0.35,
    )
...
def create_poverty_point_scenario(
    ...
    Canonical sigma = 0.45 * sqrt(20/10) = 0.636
    ...
    return EnvironmentalScenario(
        ...
        env_config=env_config,
        shortfall_params=shortfall_params,
        expected_sigma_range=(0.55, 0.70),
        expected_epsilon=0.35,  # Ecotone advantage at Poverty Point
    )


def create_critical_threshold_scenario(
    target_sigma: float = 0.57,
    name: str = "Critical Threshold",
    description: str = "Calibrated to sigma* theoretical prediction"
) -> EnvironmentalScenario:
    """
    Create scenario calibrated to produce sigma near sigma*.

    Uses canonical formula: sigma = magnitude * sqrt(20 / interval)
    Back-calculates interval from target_sigma and a fixed magnitude.

    Args:
        target_sigma: Target effective sigma value
        name: Scenario name
        description: Scenario description
    """
    # Choose magnitude based on target range, then solve for interval:
    #   interval = 20 * (magnitude / target_sigma)^2
    if target_sigma < 0.3:
        magnitude = 0.25
    ...
    interval = 20.0 * (magnitude / target_sigma) ** 2
    interval = max(3.0, min(30.0, interval))  # Sane bounds

    # Variability scales with target sigma
    ...
    return EnvironmentalScenario(
        ...
        env_config=env_config,
        shortfall_params=shortfall_params,
        expected_sigma_range=(target_sigma - 0.10, target_sigma + 0.10),
        expected_epsilon=0.35,
    )
...
SCENARIOS = {
    ...
    "critical": create_critical_threshold_scenario(target_sigma=0.57),
}
...
def list_scenarios() -> None:
    ...
    for name, scenario in SCENARIOS.items():
        ...
        print(f"  Expected sigma: {scenario.expected_sigma_range[0]:.2f} - "
              f"{scenario.expected_sigma_range[1]:.2f}")
        print(f"  Expected epsilon: {scenario.expected_epsilon:.2f}")
        sf = scenario.shortfall_params
        canonical = calculate_sigma_from_shortfall(sf.mean_interval, sf.magnitude_mean)
        print(f"  Shortfall interval: {sf.mean_interval:.0f} years")
        print(f"  Shortfall magnitude: {sf.magnitude_mean:.0%}")
        print(f"  Canonical sigma: {canonical:.3f}")
