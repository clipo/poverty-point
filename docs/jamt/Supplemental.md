...
5. *Comparison with the LiDAR-derived per-mound volume (27,065 m³).* The continuous-equilibrium prediction overshoots by a factor of $240{,}000 / 27{,}065 \approx 9\times$. The earlier 7,000 m³ figure used in pre-LiDAR drafts of the manuscript (which gave the often-cited ~30× ratio) was an under-estimate; the LiDAR re-analysis (§5.1; `scripts/analysis/watson_brake_volume.py`) replaces it. An intermediate LiDAR baseline-method sensitivity range of 15,000-120,000 m³ (the seven polygon-wide baselines in the analysis script) is documented in the JSON output for transparency, but the per-mound estimate of 27,065 m³ is the authoritative central value because it uses local rather than polygon-wide baselines and treats the mounds and ridges as resting on natural ground rather than on a constructed inter-mound surface. The same continuous-equilibrium calculation propagated through the $n_{agg}$ sensitivity sweep below confirms that no plausible $n_{agg}$ value closes the 9× gap.
...
Under the prior 7,000 m³ WB volume figure, $K = 3$ years and $\sigma_{sd} \approx 0.10$ gave a sweep-selected closure (predicted 5,408 m³ inside the prior observed value's 95% CI). Under the refined LiDAR-derived per-mound WB volume of 27,065 m³ (§5.1), the role of regime switching shifts. Across the swept $(\sigma_{sd}, K)$ grid, regime-switching predictions for cumulative WB volume span ~5,000 to ~54,000 m³ at $K \in \{1, 2, 3\}$ and $\sigma_{sd} \in [0.075, 0.20]$. The observed 27,065 m³ sits between the $K = 3$ undershoot (~5,000 m³) and the $K = 1$ overshoot (~54,000 m³) and would be bracketed by an intermediate $K = 2$ value. The per-event labor-scaling extension (§S17.2) closes the WB gap essentially exactly (~25,000 m³ predicted, 0.93× of observed) independently of any specific $K$ value sweep-selected against the volume observation. We therefore no longer treat the regime-switching closure as the principal reconciliation. The regime-switching extension's substantive role in the framework is to account for the episodic *tempo* of WB construction (the 200+-year inter-stage gaps), which it does at any $K \geq 1$ given the WB threshold proximity; this episodic structure is the prediction the framework makes about WB irrespective of the volume figure. The independent ethnographic anchoring of $K$ from multi-band cooperation timescales (Wiessner 2002:434-435; Kelly 2013:159-167) remains identified as priority follow-up.
...
Reproducible from `scripts/analysis/renovation_ratio.py`; outputs in `results/sensitivity/renovation_ratio.json`.
