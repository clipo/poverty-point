...
2. **Strategy decision:** each band computes expected $W_{agg}$ and $W_{ind}$ given current $\sigma$, $\varepsilon$, network state, and monument stock, then chooses the higher-fitness strategy with stochastic perturbation.
...
7. **Foraging and shortfall impact:** payoff modulated by $\sigma_{eff}$ for aggregators and full $\sigma$ for independents, with vulnerability $\alpha(k_i)$ applied.
8. **Reproduction:** stochastic births/deaths proportional to fitness and resources, with band fission when size exceeds 30.
...
where $|O_i|$ is the band's obligation count, $\rho$ is the fraction of potential ties realized, $\beta_{\text{net}} = 0.5$ is the network-buffering coupling strength, and $\varepsilon_{eff}$ is capped at 0.95 to prevent $\sigma_{eff} \to 0$. At canonical PP-scenario parameters ($\varepsilon = 0.35$), bands with high realized $\rho$ achieve $\varepsilon_{eff}$ up to $\sim 0.675$, a substantial increase from the base $\varepsilon$ that the $\beta_{\text{net}}$ parameter drives. The $\beta_{\text{net}} = 0.5$ choice is canonical for these runs. §S3.1 reports an OAT sensitivity sweep on $\beta_{\text{net}}$ across the range $[0.25, 0.75]$ showing a moderate threshold shift ($\Delta \sigma^{*} \approx \pm 0.02$) that does not reverse the qualitative phase-transition signature.
...
The result is clean (Figure S04). Per-cell mean dominance partitions into four bands: cells with $\varepsilon$ off plus $\lambda_W$ off are deeply independent (mean dominance $\approx -0.81$ across all four such cells); cells with $\varepsilon$ off but $\lambda_W$ on are still independent but less so ($\approx -0.54$); cells with $\varepsilon$ on but $\lambda_W$ off sit near zero ($\approx +0.05$); and cells with $\varepsilon$ on and $\lambda_W$ on are strongly aggregator ($\approx +0.47$). The $\lambda_C$ and $\lambda_X$ components make no detectable difference at any combination of $\lambda_W$ and $\varepsilon$ states. The main-effect summary makes this concrete:

| Channel | Symbol | Main effect (mean dominance ON minus OFF) |
|---|---|---|
| Within-group reward | $\lambda_W$ | $+0.339$ |
| Conflict deterrence | $\lambda_C$ | $+0.001$ |
| Cooperation-network value | $\lambda_X$ | $-0.000$ |
| Ecotone buffer | $\varepsilon$ | $+0.936$ |
...
**Cumulative selection drives the population-level shift.** At the cumulative scale, the small per-year $S(t)$ values integrate coherently along persistent band lineages and produce substantial population-level shifts. Below threshold ($\sigma = 0.20$), the post-burn-in mean $S(t) \approx -0.007$ across all replicates with no replicate-to-replicate ambiguity in sign. Cumulated over the 150-year post-burn-in interval, this produces a population shift toward independents that takes the population to $\bar{p} \approx 0.18$-$0.34$ aggregator final frequencies. Above threshold ($\sigma = 0.64$, PP-equivalent), the per-year mean $S(t) \approx +0.04$ accumulates over the same interval to produce a population shift toward aggregators that takes the population to $\bar{p} \approx 0.92$. The longer-scale population shift is therefore driven not by any one year's selection differential but by the integrated history of band-level decisions, amplified by the network-feedback loop (more aggregators → denser network → stronger pull toward aggregation in the next year). Near threshold ($\sigma = 0.40$), stochastic regime-flipping produces per-year $S(t)$ values consistently above zero (replicates land in the upper part of the threshold zone), but with high replicate-to-replicate variance characteristic of phase-transition zones.
...
| 6.1 (decay) | Distance-decay rank correlation | 500-km length scale fixed to ordering | Webb 1982 source-distance inventory | $\rho = 0.95$ but acknowledged not to discriminate against transport-cost or down-the-line alternatives |
...
| 6.3 | Watson Brake regime-switching consistent with WB scale | $\alpha = 2$, $K = 3$, $\sigma$, $\sigma_{sd}$, $\varepsilon$ | $n_{agg}$ derived independently | Consistency demonstration with three free parameters |
...
|---|---|---|---|
| 0.20 | 0.291 | $-0.414 \pm 0.080$ | $-0.401 \pm 0.063$ |
| 0.28 | 0.414 | $+0.005 \pm 0.077$ | $+0.053 \pm 0.143$ |
| 0.36 | 0.537 | $+0.433 \pm 0.112$ | $+0.454 \pm 0.106$ |
| 0.40 | 0.599 | $+0.647 \pm 0.112$ | $+0.592 \pm 0.143$ |
| 0.44 | 0.660 | $+0.783 \pm 0.074$ | $+0.749 \pm 0.084$ |
| 0.50 | 0.752 | $+0.879 \pm 0.057$ | $+0.883 \pm 0.059$ |
| 0.55 | 0.830 | $+0.927 \pm 0.041$ | $+0.927 \pm 0.036$ |

Crossovers (strategy dominance through zero, located by linear interpolation between bracketing $\sigma_{eff}$ values): **signal-conditional $\sigma_{eff}^* = 0.413$, random-partner $\sigma_{eff}^* = 0.400$.** The threshold shift between configurations is $-0.013$ in realized $\sigma_{eff}$ (a $-3.2\%$ shift), where the sign convention is random minus signal. The signal-conditional crossover matches the analytical prediction $\sigma^* = 0.40$ at $\varepsilon = 0.35$ to within 0.013. The random-partner crossover lands almost exactly on the analytical $\sigma^* = 0.40$, which is what the framework's two-component architecture (§S5.4 factorial: $\varepsilon$ and $\lambda_W$ drive the threshold; $\lambda_C$ and $\lambda_X$ are silent) predicts: random-partner removes the signal-weighting on tie formation but leaves the cooperation-network buffering and within-group reward components intact, so the threshold should land at the analytical value driven by those components.

Point-by-point dominance differences (random minus signal) at each $\sigma_{target}$: $+0.014, +0.049, +0.021, -0.056, -0.034, +0.004, +0.000$. The n=20 result tightens the n=6 result without reversing its substance: the differences alternate in sign across the transition zone (positive at low and near-threshold σ, negative at slightly-above-threshold σ, back to zero at saturation), and the magnitudes are within 1 SD of zero at every cell. Both versions converge within 0.001 of each other at the highest $\sigma_{eff}$ values (signal $+0.927$ vs random $+0.927$ at $\sigma_{target} = 0.55$, identical at the reported precision). The substantive result of §4.3, that weighted partner choice does not shift the threshold relative to a random-partner cooperation network, survives the n=20 tightening.

![](../figures/supplemental/figure_S05_ablation_n20.png)

***Figure S05. Signal-conditional vs random-partner ablation, n=20 per cell.*** *Visualization of the §S7.1 table data sourced from the round-6 ablation pad (`results/ablation/overnight_sweep.json`). (A) Strategy dominance vs realized* $\sigma_{eff}$ *for both modes with* $\pm 1$ *SD envelopes. The analytical threshold* $\sigma^{*} = 0.40$ *is the red dotted vertical line. The two S-curves are nearly indistinguishable on this scale. (B) Realized* $\sigma_{eff}$ *vs target* $\sigma_{eff}$ *sweep diagnostic. The integrated simulation's CV-adjustment monotonically raises realized values above target. (C) Per-cell dominance difference (random minus signal) with* $\pm 1$ *SE error bars. Differences alternate in sign across the transition zone and cross zero at every cell, consistent with no detectable threshold shift. (D) Crossover detail zoomed near the threshold: signal-conditional crosses at* $\sigma_{eff}^{*} = 0.413$*, random-partner at* $\sigma_{eff}^{*} = 0.400$*. Shift =* $-0.013$ *in realized* $\sigma_{eff}$ *(* $-3.2\%$*), well within replicate noise.*
...
| Cowpen Slough | 2 (Red/Black ≈ Ouachita-system + Upland partial) | 0.101 |
