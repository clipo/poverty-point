# Supplemental Material for *Signaling under environmental uncertainty: A multilevel-selection threshold model for Poverty Point*
...
Each year, each process executes exactly once, in this order:
1. **Environmental update:** advance annual zone shocks. Shortfall onset is Bernoulli with per-year probability $1/T$; event magnitude is drawn from $\mathcal{N}(m, 0.15)$ clipped to $[0.1, 0.9]$, and each event persists $\max(1, \lfloor 1 + \text{magnitude} \times \text{duration\_scale} \rfloor)$ years (duration_scale 2.0-2.5 by scenario). In shortfall years every harvest draw is reduced by the event severity, propagating the shortfall through realized productivity. Harvests are also density-dependent: the per-band share scales inversely with the current band count (normalized to the initial 50), representing finite regional productivity. The composite $\sigma$ is computed from the canonical formula $m\sqrt{20/T}$ with a small realized-variance (CV) adjustment.
2. **Strategy decision (synchronous):** every band evaluates $W_{agg}$ and $W_{ind}$ against the same start-of-year population state: the previous year's realized attendance, the site's depreciated monument signal stock $M_g$, and the signaling incentive $\lambda = \lambda_W + \lambda_C(M_g) + \lambda_X(M_g, \sigma)$ implied by that stock. The choice is a softmax on the fitness difference (temperature $\tau = 0.1$) with the $\pm 0.05$ recent-experience memory adjustment (main §3).
3. **Travel and aggregation:** aggregators incur travel cost $C_{travel} = 0.0004 d$ to the actual site location (capped at 30% of current resources), arrive at the site, and harvest ecotone resources.
4. **Monument investment:** attending bands with $R_i > 0.3$ contribute at the signaling-equilibrium level $x^*(q_i) = \sqrt{\lambda (q_i^2 - q_{min}^2)}$, multiplied by uniform noise on $[0.7, 1.3]$ and capped at $R_i \times \text{band\_size}_i \times 0.5$. Both individual prestige ($+0.1 \times$ investment) and site-level $M_g$ increment.
5. **Obligation formation (second pass, full cohort):** each attending band forms a new tie with probability $0.20 + 0.20 \, \hat{d}_i$, where $\hat{d}_i$ is its cohort-normalized display (§S1.6); the partner is drawn with weights $0.5 + \hat{d}_j$. Ties are reciprocal: both bands record the obligation.
6. **Exotic acquisition:** per-material acquisition probability follows the §S1.6 specification ($b_{base} = 0.25(1 + q_i/(1+q_i))$, distance decay $e^{-d/500}$, value/cost ratio capped at 2.0, total capped at 0.5), with at most one material acquired per band per year.
7. **Shortfall impact:** in shortfall years, bands lose members at rate $1 - \exp(-0.2 \, \alpha(k_i) \times \text{severity})$, where $\alpha(k_i)$ uses the band's realized seasonal network degree ($k_i = k_0 + \sum_j O_{ij}$, capped at $k_0 + k_{max}$ and seasonally averaged) for aggregators and $k_0$ for independents; the 0.2 scale calibrates an unbuffered band's loss to ~9-10% per event-year at PP-scenario severity. Aggregators first call obligations in descending strength order (total need 0.15); help is a resource transfer debited from the called partner.
8. **Reproduction and demography:** births are Binomial(size, $0.03 \times \text{fitness} \times (0.5 + R_i)$); deaths are Binomial(size, $0.02 \times \text{clip}(2 - \text{fitness}, 0.5, 2)$), so both are fitness-weighted. Resources above 1.0 decay by 5% (storage loss). The site's signal stock depreciates once per year, $M_g(t+1) = (1-\delta) M_g(t) + I_g(t)$ (cumulative physical fill is tracked separately and does not depreciate). Bands above 30 members fission deterministically: a daughter band takes half the members and resources and inherits the parent's monument-contribution history, prestige, and obligation network at half weight. Bands below 5 members dissolve.
...
**$\sigma$ versus $\sigma_{eff}$ naming convention.** Three closely related quantities appear throughout this article and the code, and we are explicit about each here because the convention is otherwise easy to confuse. (1) $\sigma$ is the regional composite uncertainty, defined below as $m \cdot \sqrt{20/T}$. It is the quantity that depends only on shortfall recurrence and magnitude, not on site geography. (2) $\sigma_{eff} = \sigma (1 - \varepsilon)$ is the analytical *effective* uncertainty experienced by aggregator bands at a high-$\varepsilon$ gathering site. It is what enters the survival term in `aggregation_expected_fitness` (signaling_core.py) and the analytical threshold-proximity computation. (3) The sweep variable in the §4.3 ablation and §S7.1 tables, labeled either "$\sigma_{eff}$ target" or "realized $\sigma_{eff}$" depending on context, is on the same numerical scale as $\sigma$ rather than $\sigma_{eff} = \sigma(1-\varepsilon)$: the script `signal_conditional_ablation_sweep.py` inverts the relation $\sigma_{eff,\text{target}} = m \sqrt{20/T} (1-\varepsilon)$ for the recurrence $T$ that achieves the target, runs the simulation at that $T$, and reports `mean_effective_sigma` from `IntegratedSimulation._calculate_effective_sigma`, which returns the raw $\sigma$ (with a small CV adjustment) rather than $\sigma(1-\varepsilon)$. The discount $(1-\varepsilon)$ is then applied inside the analytical fitness call. The §S7.1 table labels and Figure 7 x-axis therefore report a quantity that is on the raw-$\sigma$ scale even though they use the "$\sigma_{eff}$" label. The analytical threshold $\sigma^{*} = 0.40$ for $\varepsilon = 0.35$ is also on the raw-$\sigma$ scale (because `critical_threshold(epsilon=0.35)` returns the analytical $\sigma^{*}$ at which the survival differential vanishes, evaluated against raw $\sigma$ entering the discount). Within the §S7.1 / Figure 7 analysis, the convention is therefore internally consistent: the sweep variable, the threshold, and the realized values are all on the raw-$\sigma$ scale, and the label "$\sigma_{eff}$" is shorthand for "the raw-$\sigma$ value at which the discount-adjusted survival term would correspond to ecotone advantage $\varepsilon = 0.35$." Outside of §S7.1 / Figure 7, when we write $\sigma_{eff}$ in the main text we mean the analytical $\sigma (1-\varepsilon)$ quantity proper. The contexts are clearly separated by which table or figure is being referenced.
...
Simulation begins with 50 bands distributed uniformly across the regional landscape. Each band's initial strategy is Bernoulli(0.5) aggregator/independent, its quality is drawn from $U[q_{min}, q_{max}]$, its size from $25 \pm 5$ members, and its resources from $U[0.4, 0.6]$. Each band is seeded with 0-2 weak reciprocal obligations (strength 0.1) to randomly chosen partners; monument stock starts at zero. Spin-up of 50 years is discarded before metric collection.
...
*Signal-conditional partner formation.* For each band $i$ attending the aggregation, a display level $d_i$ is computed by summing two quantities on different native scales: $\text{display}_i = M_i + E_i$, where $M_i$ is the band's cumulative monument-contribution score (continuous fitness-fraction units; on the order of $10^0$ to $10^1$ at PP scale) and $E_i$ is the band's total exotic-item count (integer count; on the order of $10^1$ to $10^3$ at PP scale). The additive combination places these on the same axis without scale-matching, which we acknowledge as a model-implementation choice rather than a derivation. The combined display is then normalized within the attending cohort, $d_i / \max_j d_j$, and the per-band tie-formation probability is $0.20 + 0.20 \cdot d_i / \max_j d_j$ (so high-display bands form ties at $\sim 40\%$ per aggregation versus $\sim 20\%$ for the lowest-display attendees). When a tie forms, the partner is drawn from co-attending bands with weights $0.5 + d_j / \max_k d_k$, so candidates' displays scale partner-choice weight from $0.5\times$ for low-display partners to $1.5\times$ for max-display partners. The additive $M + E$ combination matters quantitatively because at high-exotic-count bands the integer exotic count can numerically dominate the monument contribution. This is partially offset by the per-cohort normalization (each cohort has its own scale). A sensitivity probe in §S3.1 reports the threshold-location robustness to an alternative formulation where $M$ and $E$ are separately z-scored before summing.

*Tie-strength-weighted shortfall help.* When an aggregator calls obligations during a shortfall (total need = 0.15 in normalized resource units), obligations are sorted by strength and called in descending order until the need is met or the obligation network is exhausted. Each call decays the called obligation by $\times 0.7$ and removes it if strength falls below 0.05 (existing `call_obligation` behavior; help_received = $\min$(remaining_need, strength $\times 0.5$)).
...
$$\varepsilon_{eff} = \varepsilon + (1-\varepsilon) \cdot \beta_{\text{net}} \cdot \rho, \quad \rho = \frac{|O_i|}{n_{attending} - 1}$$

where $|O_i|$ is the band's obligation count, $\rho$ is the fraction of potential ties realized, $\beta_{\text{net}} = 0.5$ is the network-buffering coupling strength, and $\varepsilon_{eff}$ is capped at 0.95 to prevent $\sigma_{eff} \to 0$. At canonical PP-scenario parameters ($\varepsilon = 0.35$), bands with high realized $\rho$ achieve $\varepsilon_{eff}$ up to $\sim 0.675$, a substantial increase from the base $\varepsilon$ that the $\beta_{\text{net}}$ parameter drives. The $\beta_{\text{net}} = 0.5$ choice is canonical for these runs. §S3.1 reports an analytical sensitivity computation across $\beta_{\text{net}} \in [0, 1]$ (worst-case fully connected $\rho$; $\sigma^{*}$ ranges from 0.40 to 0.26) showing that no choice of $\beta_{\text{net}}$ reverses the qualitative phase-transition signature.

*Exotic acquisition.* During each aggregation year, each aggregator band iterates through the available exotic material classes (copper, steatite, galena, novaculite, crystal quartz) and computes a per-material acquisition probability:

$$p_{\text{acq},m} = b_{\text{base}} \cdot \exp\!\left(-\frac{d_m}{\lambda_d}\right) \cdot \min\!\left(\frac{v_m}{c_m + 0.01},\; 2.0\right)$$

where $b_{\text{base}} = 0.25 \cdot (1 + q/(1+q))$ with $q$ the band's quality (so $b_{\text{base}} \in [0.25, 0.50]$), $d_m$ is the distance from the source, $\lambda_d = 500$ km is the distance-decay scale, $v_m$ is the per-material signal value computed from source distance ($v_m = \rho_{exotic} \ln(1 + d_m/100)$; `exotic_signal_value`), and $c_m$ is the per-material acquisition cost ($c_m = 0.05\sqrt{d_m/250}$; `exotic_signaling_cost`). The value/cost ratio is capped at $2.0$ to prevent any single low-cost material from saturating the per-band acquisition budget. The total $p_{\text{acq},m}$ is then capped at $0.5$, and after the first successful acquisition in a given material the loop breaks (one material per aggregation year per band). This per-call cap is a model-implementation choice rather than a parameter. It bounds the per-year acquisition rate to one material acquisition event regardless of how strongly the per-material probabilities multiply. Per-band cumulative exotic counts at the end of the simulation reflect the integral of these per-year per-material acquisition events.

The signal-blind alternative remains available in the code via the `signal_conditional_partners` flag for ablation comparisons: the same expected number of ties forms per pass (a uniform tie probability equal to the cohort mean of the conditional probabilities), but who pairs with whom is uniform random; shortfall help is a single random obligation call; and the $\sigma_{eff}$ network-density coupling is disabled. The default in current calibration runs is signal-conditional mode.
...
| $I_g$ | Per-year monument investment flow | investment units / yr | $\sum_i \text{invest}_{i,t}$ (summed over bands in year $t$) | $M_g$ |
...
| $B(\lambda)$ | Signaling benefit (mean-field) | fitness units | $(\lambda/2)(q_{min}+q_{max})/2 + \lambda q_{min}^2 \ln(q_{max}/q_{min}) / (2(q_{max}-q_{min}))$ | $W_{agg}$ |
...
| $\mu_c$ | Conflict mortality fraction | $[0, 1]$ | 0.08 | $W$ |
...
Plainly: B(λ) is the average extra fitness a band gets from signaling, computed by averaging across the range of band qualities present in the population. The first term is the leading contribution. The second is a small correction that accounts for the curvature of the per-band benefit function near $q_{min}$. At our parameters ($q_{min}=0.2$, $q_{max}=2.0$; the ratio of the two terms is independent of $\lambda$) the second term is approximately 5% of the first, and the code computes both.

### Conflict parameters

Baseline annual conflict probability $P_{base} = 0.008$ and conflict mortality $\mu_c = 0.08$ are set conservatively low, reflecting reduced territorial fixity of mobile hunter-gatherers relative to sedentary populations. Monument investment reduces conflict through an assessment mechanism (Enquist and Leimar 1983).
...
| Conflict mortality | $\mu_c$ | 0.08 | 0.04-0.15 | ethnographic |
...
Reproducible from `scripts/analysis/near_threshold_ablation.py`, with outputs in `results/sensitivity/near_threshold_ablation.json`. The full ABM-level near-threshold sweep (with stochastic regime flipping rather than analytical equilibrium) remains as future work.
...
The choice of $\xi_X = 0.5$ as a defensible upper bound is informed by the Bliege Bird and Smith (2005) framework where signal quality multiplies per-partnership value. Further empirical grounding via ethnographic data on signal-quality-mediated partner choice premia would tighten the parameter. Reproducible from `scripts/analysis/restructured_saturation_test.py`, with outputs in `results/sensitivity/restructured_saturation_test.json`.

### S5.4 Factorial component ablation

R3's review (round-6) requested a deeper ablation than the §4.3 signal-conditional-vs-random partner comparison: turn each signaling/buffering component off individually and in factorial combination to isolate which components actually drive the threshold-crossing. We ran a $2^4 = 16$-cell factorial across the four components, with 8 stochastic replicates per cell at sigma-effective target 0.40, just above the analytical threshold. The four components are $\lambda_W$ (within-group reward), $\lambda_C$ (conflict deterrence), $\lambda_X$ (cooperation-network value), and $\varepsilon$ (ecotone buffer). A component is "on" at its canonical PP-scenario value (0.15, 0.10, 0.15, 0.35 respectively) and "off" at zero.

The result is clean (Figure S6). Per-cell mean dominance partitions into four bands: cells with $\varepsilon$ off plus $\lambda_W$ off are deeply independent (mean dominance $\approx -0.72$ across all four such cells); cells with $\varepsilon$ off but $\lambda_W$ on are still independent but less so ($\approx -0.43$); cells with $\varepsilon$ on but $\lambda_W$ off are moderately aggregator ($\approx +0.25$); and cells with $\varepsilon$ on and $\lambda_W$ on are strongly aggregator ($\approx +0.61$). The $\lambda_C$ and $\lambda_X$ components make no detectable difference at any combination of $\lambda_W$ and $\varepsilon$ states. The main-effect summary makes this concrete:

| Channel | Symbol | Main effect (mean dominance ON minus OFF) |
|---|---|---|
| Within-group reward | $\lambda_W$ | $+0.323$ |
| Conflict deterrence | $\lambda_C$ | $-0.012$ |
| Cooperation-network value | $\lambda_X$ | $+0.011$ |
| Ecotone buffer | $\varepsilon$ | $+1.003$ |

The headline finding is that *ecotone access* and *within-group reward* are the two components that drive the threshold-crossing structure. The conflict-deterrence and cooperation-network-value components make no detectable contribution at the PP-scenario parameterization. This is consistent with the §S3.1 OAT sensitivity result: $\lambda_C$ and $\lambda_X$ have zero direct sensitivity in the OAT tornado because they are endogenously determined at the lambda-sigma fixed point, and the factorial confirms that endogenization extends to the full ABM dynamic (turning them off entirely produces no detectable change in the population trajectory at this parameterization). The framework's two operative components at PP-scenario parameters are $\varepsilon$ (which sets whether the threshold is crossed) and $\lambda_W$ (which sets how far above the threshold the system equilibrates). $\lambda_C$ and $\lambda_X$ are present in the model architecture but doing no observable work at these parameter values. A different parameterization (e.g., earlier in the analytical fixed-point iteration before $M_g$ saturates the network function) would presumably give them more weight. At the PP equilibrium they are silent.

This factorial ablation is the deeper version of the §4.3 signal-conditional vs random-partner comparison. The §4.3 result ($-0.002$ shift in realized $\sigma_{eff}$ at the threshold) tested whether *weighted partner choice* matters when network cooperation is preserved, and found that it does not: the threshold does not shift. The §S5.4 factorial here tests whether each *component* matters, with everything else held constant. Here, $\varepsilon$ and $\lambda_W$ matter substantially while the other two do not at PP-scenario parameters. The two results together give a more complete picture of which model components are doing the work and which are present but not currently engaged.

![](../../figures/supplemental/figure_S06_factorial_ablation.png)

***Figure S6. Factorial component ablation.*** *Per-cell mean dominance (mean ± 1 SD across 8 stochastic replicates) for the 16-cell factorial across* $\lambda_W$*,* $\lambda_C$*,* $\lambda_X$*, and* $\varepsilon$ *(uppercase = component on at PP-scenario value; lowercase = off at zero). Cells are color-coded by* $\varepsilon$ *state (orange = on, blue = off). The four bands of dominance values correspond to the four combinations of* $\varepsilon$ *and* $\lambda_W$ *states. The* $\lambda_C$ *and* $\lambda_X$ *components make no detectable difference within each band. (B) Main effects: average dominance with component ON minus average with component OFF, marginalized over the other three components.* $\varepsilon$ *(+0.94) and* $\lambda_W$ *(+0.34) drive the threshold-crossing dynamic. The conflict-deterrence and cooperation-network components are effectively silent at PP-scenario parameters because they are endogenously determined at the lambda-sigma fixed point (cf. §S3.1).*

### S5.5 Empirical Price-equation decomposition from ABM output (band-level + cumulative)

Main §2.1 describes the framework's two-scale architecture: per-year band-level decisions (each band chooses its strategy on individual-fitness grounds) coupled through frequency-dependent network feedback (band fitness depends on emergent population-level network density). The Price equation,

$$\Delta\bar{p} = \frac{\mathrm{Cov}(w_g, p_g)}{\bar{w}} + \frac{E(w_g \, \Delta p_g)}{\bar{w}},$$

provides a population-dynamics accounting tool for tracking year-on-year change in the aggregator frequency $\bar{p}$. For the binary aggregator/independent strategy in this model, where each band is wholly one or the other, the within-group selection term collapses to zero (no within-band strategy variation) and the equation reduces to a between-strategy-class selection differential $S(t)$ plus a transmission residual. $S(t)$ is positive whenever aggregator bands are out-reproducing independent bands at year $t$ and negative when the reverse holds. The cumulative integral $\sum_t S(t)$ tracks how far the population frequency has shifted as a result. To verify that the agent-based model exhibits the expected band-level-selection plus network-feedback dynamic, we compute $S(t)$ empirically from per-band simulation output across three $\sigma$ scenarios spanning the framework's threshold.

**Setup.** Three scenarios at PP-equivalent $\varepsilon = 0.35$: below-threshold ($\sigma_{target} = 0.20$), near-threshold ($\sigma_{target} = 0.40$), and above-threshold ($\sigma_{target} = 0.64$, PP-equivalent). Each scenario is run for 5 stochastic replicates × 200 simulated years with a 50-year burn-in discarded. After each year, per-band state is snapshotted (strategy, fitness, monument contribution, exotic count, network position) and the year-on-year Price decomposition is computed: the between-strategy-class selection term $S(t)$, the within-aggregator-cohort covariance $\mathrm{Cov}(M_i, w_i)$ across bands in the aggregator class (which indexes whether more monument investment is rewarded individually within the cohort), and the transmission residual $\Delta\bar{p} - S(t)$ (capturing strategy switching beyond what reproductive differential alone delivers). Reproducible from `scripts/analysis/run_price_decomposition.py`, with outputs in `results/analysis/price_decomposition_*.json`. Figure S7 reports per-year trajectories, the cumulative-$S(t)$ integral, and post-burn-in summary statistics.

**Per-year selection differential is small but consistent in sign.** At the per-year resolution, $S(t)$ is small in all three regimes: $|S| < 0.10$ throughout. This is the expected result under the framework's logic. Per-year fitness differentials between aggregator and independent bands are bounded by the buffering value of the network at that year's realized shortfall: a single year cannot produce large strategy-class fitness differences when both classes face the same regional environment. The dominant engine of per-year band decisions is band-level individual fitness. Each band picks its strategy based on its own expected payoff, and the per-year selection signature is what falls out of those decisions in aggregate.

**Cumulative selection drives the population-level shift.** At the cumulative scale, the small per-year $S(t)$ values integrate coherently along persistent band lineages and produce substantial population-level shifts. Below threshold ($\sigma = 0.20$), the post-burn-in mean $S(t) \approx -0.011$ across all replicates with no replicate-to-replicate ambiguity in sign. Cumulated over the 150-year post-burn-in interval, this produces a population shift toward independents that takes the population to $\bar{p} \approx 0.30$-$0.41$ aggregator final frequencies. Above threshold ($\sigma = 0.64$, PP-equivalent), the per-year mean $S(t) \approx +0.02$ accumulates over the same interval to produce a population shift toward aggregators that takes the population to $\bar{p} \approx 0.98$. The longer-scale population shift is therefore driven not by any one year's selection differential but by the integrated history of band-level decisions, amplified by the network-feedback loop (more aggregators → denser network → stronger pull toward aggregation in the next year). Near threshold ($\sigma = 0.40$), stochastic regime-flipping produces per-year $S(t)$ values consistently above zero (replicates land in the upper part of the threshold zone), but with high replicate-to-replicate variance characteristic of phase-transition zones.

**Within-aggregator individual fitness signal.** The within-aggregator $\mathrm{Cov}(M, w)$ term is small but consistently positive above threshold and at near-threshold conditions: bands with higher cumulative monument contributions have slightly higher fitness within the aggregator cohort. This is consistent with the framework's prediction that monument-investment quality scales partner-choice premium and network buffering at the individual band level. The within-cohort signal is modest because individual fitness benefits already drove the strategy choice (the bands now in the aggregator cohort are those for which aggregating was individually optimal at the moment of choice), so additional within-cohort fitness variation reflects only secondary differences in band quality, network position, and accumulated monument-investment history.

**Why persistent kin-bands matter for the cumulative dynamic.** The model's bands are persistent units across generations: bands fission at size 30 with daughter bands inheriting half the parent band's network, network ties persist across years subject to decay, and band identity tracks across the simulated interval. This persistence is what makes the cumulative $S(t)$ integral coherent. In a fluid-coalition model where band membership reshuffles each year, the per-year selection differentials would scramble across generations and the cumulative population shift would not accumulate as efficiently. In the persistent-kin-band model the small per-year selection differentials compound coherently along band lineages, and the integrated signal is what produces the equilibrium population-level outcome the framework predicts.

**Numerical summary** (5-replicate × 200-year decomposition per scenario, post-burn-in pooled across the 50-200 yr window; full per-replicate output in `results/analysis/price_decomposition_20260820_151849.json`):

| Scenario | $\sigma_{target}$ | $\bar{p}$ (final, mean across reps) | $S(t)$ per-year mean ± SD | Within-agg $\mathrm{Cov}(M, w)$ |
|---|---|---|---|---|
| Below threshold | 0.20 | 0.368 (range 0.30-0.41) | $-0.0113 \pm 0.0042$ | $+0.053$ |
| Near threshold | 0.40 | 0.814 (range 0.74-0.88) | $+0.0467 \pm 0.0077$ | $+0.094$ |
| Above threshold (PP) | 0.64 | 0.978 (range 0.96-0.99) | $+0.0226 \pm 0.0125$ | $+0.184$ |

Three features of this table are worth pointing at directly. First, the per-year $S(t)$ is small in absolute value at every scenario, consistent with the framework's expectation that single-year fitness differentials between strategy classes are bounded by realized shortfall buffering. Second, the sign of $S(t)$ is what flips across the threshold: negative below, strongly positive at and above. The per-replicate consistency is tight (the standard deviations are an order of magnitude smaller than the means). This is the band-level selection signature the framework predicts. Third, the within-aggregator $\mathrm{Cov}(M, w)$ is positive in all three regimes and grows with $\sigma$ (from $+0.05$ below threshold to $+0.18$ above it): bands with higher cumulative monument contributions have higher realized fitness within the aggregator cohort, and the association strengthens as shortfalls become more frequent because network buffering (which tracks accumulated display through signal-conditional partner formation) is exercised more often. This is the within-cohort signature of the signaling channel operating through partner choice, consistent with the framework's frequency-dependent feedback architecture.

The cumulative $\sum S(t)$ over the 150-year post-burn-in interval is approximately $-1.7$ at low $\sigma$, $+7.0$ at the threshold, and $+3.0$ at PP-scenario. These integrate-of-$S(t)$ values are not directly interpretable as population frequency shifts (because once $\bar{p}$ saturates near 0 or 1, the population-state bounds further accumulation), but the relative magnitudes confirm the cumulative dynamic: the band-level selection signature, small year by year, compounds along persistent band lineages to produce the equilibrium population frequencies the simulation actually exhibits.

![](../../figures/supplemental/figure_S07_price_decomposition.png)

***Figure S7. Empirical Price-equation decomposition of band-level selection across the framework's threshold.*** *(A) Per-year between-strategy-class selection differential* $S(t) = \mathrm{Cov}(w_g, p_g)/\bar{w}$ *(10-yr bins, ±1 SD across stochastic replicates), for below-threshold (* $\sigma = 0.20$*), near-threshold (* $\sigma = 0.40$*), and above-threshold (* $\sigma = 0.64$*, PP-equivalent) scenarios. Vertical red dotted line marks the burn-in cutoff at year 50. Per-year* $S(t)$ *values are small in all three regimes because the per-year fitness differential between aggregator and independent bands is bounded by realized shortfall buffering. (B) Cumulative* $\sum S(t)$ *from burn-in onward (median + 10-90 percentile envelope across replicates), demonstrating that small per-year selection differentials integrate coherently along persistent band lineages to produce substantial population-level shifts in opposite directions below and above* $\sigma^{*}$*. (C) Transmission residual* $\Delta\bar{p} - S(t)$ *per year, capturing strategy switching beyond reproductive differential. (D) Post-burn-in mean ± SD of the three Price components per scenario: between-strategy-class selection* $S$ *(blue), transmission residual (orange), within-aggregator covariance* $\mathrm{Cov}(M, w)$ *(green). The sign reversal of* $S$ *between the below- and above-threshold scenarios, accumulated over many decision cycles as in Panel B, is what drives the population convergence on the framework's predicted equilibrium. The small positive within-aggregator covariance is the residual individual-level signal that operates within the aggregator cohort.*
...
| 0.0-0.3 | Absent or trivially accessible | Claiborne's coastal setting (mast 0.3; upland 0.5 under the corrected coding that credits the Coastal Plain Upland Gulf Flatwoods); prairie zone for most LMV-interior sites |
...
| J.W. Copes (16MA47) | 0.5 | 0.6 | 0.4 | 0.7 | 0.0 |
| Cowpen Slough (16CT147) | 0.9 | 0.5 | 0.8 | 0.7 | 0.0 |
| Jaketown (22HU505) | 1.0 | 0.3 | 0.8 | 0.5 | 0.0 |
| Claiborne (22HA501) | 1.0 | 0.5 | 0.5 | 0.3 | 0.0 |
| Cedarland (22HA506) | 1.0 | 0.5 | 0.5 | 0.3 | 0.0 |
...
### S7.0 Status summary of §6 empirical evaluations

The eight empirical evaluations in main §6 differ substantially in inferential weight. The table below summarizes, for each evaluation, the section in which it appears, what it tests, what parameters are fit to the data, what data are treated as independent of the model fit, and the resulting status. The bottom row reports the §4.3 signaling-vs-cooperation ablation comparison, which is theoretical rather than empirical but is listed here because its result affects how the empirical evaluations should be interpreted (the threshold-location dimension does not discriminate signaling from generic cooperation; the spatial-flow signature at §6.1 does).

| Section | Evaluation | What is fit | What is independent | Status |
|---|---|---|---|---|
| 6.1 (vol) | Type-site volume calibration | ~104 m³/unit scaling factor fit to PP target | None (calibration anchor) | Calibration; not a test |
| 6.1 (exotic) | Exotic-goods count consistency | Same simulation run as 6.1 vol | Per-material counts (different model pathway for each) | Partial test; consistent for copper and galena in like units and for steatite on a vessel basis; near-source materials (novaculite, crystal quartz) overpredicted 8-63× by the cost-driven acquisition rule (genuine misfit); joint Mahalanobis rejects |
| 6.1 (decay) | Distance-decay rank correlation | 500-km length scale fixed a priori | Webb 1982 / Gibson and Griffing 1994 source-distance inventory | $\rho = -0.40$ across five materials (+0.50 far-source only); fails as an unconditional ordering prediction, and acknowledged not to discriminate against transport-cost or down-the-line alternatives in any case |
| 6.1 (outflow) | Asymmetric flow signature: into-PP, not out-from-PP | None (the asymmetry is a framework prediction) | Webb 1982 type-site inventory; Smith 1991 cross-site material-share percentages; Kidder and Grooms 2025 outflow observation | Discriminating test passed: signaling reading predicts asymmetric flow into the gathering; risk-pooling predicts bidirectional flow; down-the-line exchange predicts smooth distance-decay; neither alternative is observed. Coarse pass on existing data; finer-grained patterning across visitor-band-identified sites is the §7.4 sharpening study |
...
| 6.3 | Watson Brake volume closure via per-event labor scaling | $\alpha = 2$ (fitted); $K$ accounts for tempo, not volume | $n_{agg}$ derived independently | Consistency demonstration conditional on the fitted labor-scaling exponent |
| 6.4 | Cross-LMV interior-vs-coastal screening | Static-rubric weights | LMV site catchment geomorphology (interior monument-building + non-mound comparator sample) | Sample-selection-bounded |
| 6.5 | Multi-drainage shortfall buffering of contemporary regimes | Modern USGS gauge correlations | None (descriptive; modern-as-proxy concession) | Descriptive; modern-as-proxy concession |
| 6.6 | Cross-site magnitude $M_g(\varepsilon,n_{agg})$ | $n_{agg}$ from convergence-model literature | $\varepsilon$ from rubric | Joint $\rho = + 0.85$ to $+ 0.92$, but partial-correlation: $\varepsilon$ contributes at most $\pm 0.02$ |
...
| 6.8 | Construction tempo and pulsed accumulation; maintenance-to-new-construction event density | $\delta = 0.08$ assumed (Table S2 range $[0.04, 0.16]$); $n_{agg}$ scenarios from §6.6 | PP and WB event counts (Clay 2023; Hargrave et al. 2021; Kidder et al. 2021; Saunders et al. 2005); $R(M_g)$ derivation from geometric depreciation | Pulsed-accumulation reading consistent with framework but not discriminating; the $R(M_g)$ event-density contrast (PP $\approx 1.3$ events/yr vs WB $\approx 0.02$-$0.03$ events/yr, 47-85$\times$ ratio) is independently weighted: it follows from the signal-half-life mechanism without additional free parameters, and a reversal or order-of-magnitude reduction of the contrast would weaken the mechanism. Listed as falsifiable prediction §7.4(h) |
| 4.3 | Signaling-vs-cooperation ablation (signal-conditional vs random partners) | $\sigma_{eff}$ controlled via ShortfallParams.mean_interval matched $k_{agg}$ | None; direct comparison | **Threshold shift $- 0.002$ in $\sigma_{eff}$ ($- 0.4\%$); no discriminating shift.** Signaling apparatus does not move the threshold relative to cooperation-network mechanics with random partner formation. Empirical adjudication runs through §6.1 outflow asymmetry, not threshold location |

### S7.1 Phase-transition sweep (§4.1 and §4.3)

The phase-transition validation in main §4.1 and the signal-conditional vs random-partner ablation in main §4.3 are both produced from a single sweep across seven $\sigma_{target}$ values at PP-scenario parameters ($\varepsilon = 0.35$, pinned at the aggregation site): $\sigma_{target} \in \{0.20, 0.28, 0.36, 0.40, 0.44, 0.50, 0.55\}$, with **twenty stochastic replicates per cell** (canonical output at `results/ablation/ablation_sweep_engine2.json`), 200 simulated years per replicate, and a 50-year burn-in. The sweep controls $\sigma_{eff}$ through the shortfall recurrence $T$ at fixed magnitude $m = 0.5$ (§S1.4 naming convention); realized $\sigma$ runs roughly 45-50% above target because shortfall events persist for multiple years and the realized-variance (CV) adjustment raises the composite index. Realized values are reported alongside targets throughout, and the crossings below are located on the realized scale.

Per-cell strategy-dominance statistics for both configurations (n = 20 replicates per cell, mean ± sample SD with ddof = 1):

| $\sigma_{target}$ | realized $\sigma_{eff}$ | dominance (signal-conditional) | dominance (random-partner) |
|---|---|---|---|
| 0.20 | 0.290 | $-0.370 \pm 0.074$ | $-0.368 \pm 0.055$ |
| 0.28 | 0.413 | $+0.101 \pm 0.142$ | $+0.108 \pm 0.109$ |
| 0.36 | 0.536 | $+0.480 \pm 0.066$ | $+0.469 \pm 0.056$ |
| 0.40 | 0.598 | $+0.617 \pm 0.039$ | $+0.595 \pm 0.041$ |
| 0.44 | 0.659 | $+0.711 \pm 0.034$ | $+0.706 \pm 0.039$ |
| 0.50 | 0.752 | $+0.839 \pm 0.011$ | $+0.838 \pm 0.017$ |
| 0.55 | 0.829 | $+0.891 \pm 0.017$ | $+0.901 \pm 0.011$ |

Crossovers (strategy dominance through zero, located by linear interpolation between bracketing realized-$\sigma_{eff}$ values): **signal-conditional $\sigma_{eff}^* = 0.387$, random-partner $\sigma_{eff}^* = 0.385$.** The threshold shift between configurations is $-0.002$ in realized $\sigma_{eff}$ (a $-0.4\%$ shift), where the sign convention is random minus signal. The signal-conditional crossover matches the analytical prediction $\sigma^* = 0.40$ at $\varepsilon = 0.35$ to within 0.013. The random-partner arm forms the same expected number of ties per pass (uniform probability equal to the cohort mean of the signal-conditional probabilities) with uniform random pairing, so the comparison isolates weighted partner choice while holding tie counts fixed.

Point-by-point dominance differences (random minus signal) at each $\sigma_{target}$: $+0.002, +0.007, -0.011, -0.022, -0.005, -0.001, +0.010$. The differences alternate in sign across the transition zone and are within one replicate SD of zero at every cell. Both versions converge within 0.010 of each other at saturation.

![](../../figures/supplemental/figure_S08_ablation_n20.png)

***Figure S8. Signal-conditional vs random-partner ablation, n=20 per cell.*** *Visualization of the §S7.1 table data (`results/ablation/ablation_sweep_engine2.json`). (A) Strategy dominance vs realized* $\sigma_{eff}$ *for both modes with* $\pm 1$ *SD envelopes. The analytical threshold* $\sigma^{*} = 0.40$ *is the red dotted vertical line. The two S-curves are nearly indistinguishable on this scale. (B) Realized* $\sigma_{eff}$ *vs target* $\sigma_{eff}$*, documenting the realized-vs-target adjustment described above.*

Full per-replicate output is archived in `results/ablation/ablation_sweep_engine2.json`. Earlier sweeps against the pre-revision engine (`overnight_sweep.json`, `phase_transition_summary.json`) are retained in git history for provenance but do not correspond to the current engine.
...
| Scenario | Nominal $\sigma$ | Realized $\sigma$ (mean) | Mean aggregation size (bands; population grows to ~130-190 bands under fission) | Monument units (mean ± sample SD) | Total exotics (mean ± sample SD) |
|---|---|---|---|---|---|
| Low Uncertainty | 0.32 | 0.29 | 54-125 (near-threshold mixed) | 3,402 ± 1,906 | 6,938 ± 1,749 |
| Calibrated PP | 0.64 | 0.62 | 77-188 (aggregator-dominant) | 7,203 ± 3,276 | 10,883 ± 3,131 |
| Critical Threshold | 0.57 | 0.55 | 73-180 | 6,350 ± 2,777 | 10,091 ± 3,438 |
| High Uncertainty | 1.00 (clipped to 0.95) | 0.95 | 39-118 (demographically stressed) | 6,440 ± 2,268 | 4,609 ± 2,336 |
...
**P5.** Distance-decay function $p(d) = \exp(-d/500)$: acquisition probability declines with source distance, so near-source materials should outnumber far-source materials item for item. The current Poverty Point inventory does not show this ordering unconditionally (novaculite 101 items at ~250 km versus steatite 2,221 at ~800-900 km; five-material $\rho = -0.40$, §6.1). The prediction holds only among the three far-source materials (galena, steatite, copper; $\rho = +0.50$) and after material-specific value and recovery corrections, and it is not discriminating in any case (§7.4(c)).
...
**P12.** Under the $\varepsilon$-degradation collapse pathway (main §7.3), collapse should follow a specific sequence: declining aggregation attendance precedes declining construction investment, which in turn precedes narrowing exchange-network diversity. Sequential rather than simultaneous decline distinguishes threshold-driven adaptive collapse from catastrophic disruption; under the abrupt-disruption pathway also allowed by §7.3, the three declines can be near-simultaneous, and the observed ordering is the discriminating unknown between the two pathways (§7.4(d)).
...
The expanded paleoclimate proxy synthesis (with cross-case $\sigma$ comparison and theoretical phase-space panel) and the regional chronological synthesis are presented in main-text Figures 11 and 10 respectively.

![](../../figures/supplemental/figure_S11_obligation_network.png)

***Figure S11. Obligation network structure and function.*** *Panel A: simulated obligation network at year 300. Panel B: aggregators consistently maintain more reciprocal connections than independents. Panel C: bands with more extensive obligation networks achieve higher fitness during shortfall years.*

---

## S10. Watson Brake bistable analysis

### S10.1 Derivation of the continuous-equilibrium overshoot

The main text (§6.3) reports that the calibrated model under continuous-equilibrium operation predicts ~240,000 m³ of earthwork at WB, compared to the LiDAR-derived per-mound volume of 27,065 m³ (§5.1), an overshoot ratio of approximately 9×. The derivation of the 240,000 m³ continuous-equilibrium prediction, omitted from the main text for length, is:
...
5. *Comparison with the LiDAR-derived per-mound volume (27,065 m³).* The continuous-equilibrium prediction overshoots by a factor of $240{,}000 / 27{,}065 \approx 9\times$. The same continuous-equilibrium calculation propagated through the $n_{agg}$ sensitivity sweep below confirms that no plausible $n_{agg}$ value closes the 9× gap. Methodology and per-cluster breakdown for the 27,065 m³ figure are in §5.1. The analysis script is `scripts/analysis/watson_brake_volume.py`.
...
5. *Reduction of structure to a single number.* The Shannon index reduces five zone weights to one scalar, washing out structural differences (one-dominant vs. evenly-distributed) that may matter for sustained aggregation. The EPA Level IV ecoregion measurement (S12) shows that two operationalizations of "ecotone diversity" give different rankings: Caney intersects the highest Level IV count (9), with Cowpen Slough (8) and Watson Brake (7) next, while Jaketown's catchment is dominated by Northern Holocene Meander Belts (73a; ~81% of the 25 km buffer) consistent with its Yazoo Basin meander-belt setting. Yet Watson Brake's monument scale is mid, Caney's mid, Frenchman's Bend's small, and Poverty Point's largest by orders of magnitude despite a more modest L4 count (4). There is no single $\varepsilon$ that captures the relevant ecological-diversity dimension.
...
Two independent GIS-based diversity indices applied to each of the eleven Table 2 sites produce different rankings. Under the corrected zone coding (Table 2, coastal $\varepsilon = 0.40$), the qualitative rubric correlates significantly with the Saucier-geomorphic index ($\rho = 0.70$, $p = 0.016$) but not with the Level IV ecological index ($\rho = 0.12$, $p = 0.73$). Full detail is given here; Table 3 in the main text reports the corresponding correlations against monument scale.
...
Qualitative $\varepsilon$ correlates moderately and significantly with GIS-measured geomorphic $\varepsilon$ (Spearman $\rho = 0.70$, $p = 0.016$), confirming that the rubric captures the same gross depositional-age heterogeneity. It does not track the fine-grained EPA-L4 $\varepsilon$, however ($\rho = 0.12$, $p = 0.73$). The contrast is one of scale: the qualitative rubric and the Saucier-geomorphic index both work at the coarse level of depositional-age classes, whereas the L4 classification subdivides the same landscape into many fine ecoregion types, and neither captures the canoe-day water-route catchment that the framework's $\varepsilon$ actually encodes (see §S17.4). The EPA L4 ranking is informative: Caney's catchment intersects the highest count of distinct Level IV ecoregions (9), followed by Cowpen Slough (8) and Watson Brake (7); Frenchman's Bend and Insley each intersect 6; Jaketown, Claiborne, and Cedarland each intersect 5; Poverty Point and Lower Jackson each intersect 4; and J.W. Copes intersects 2. Jaketown's catchment is dominated by Northern Holocene Meander Belts (73a; ~1,590 km² of the 1,963 km² buffer, ~81%) and Northern Backswamps (73d; ~356 km²), consistent with its published Yazoo Basin meander-belt setting (Ford, Phillips, and Haag 1955). The coastal Pearl River sites (Claiborne and Cedarland) intersect 5 ecoregions each, one more than Poverty Point's 4. The L4 measurement therefore ranks several interior sites (Caney, Cowpen Slough, Watson Brake) above Poverty Point at the 25-km scale, even though Poverty Point's monument scale is two orders of magnitude larger. The static measures agree that the high-$\varepsilon$ band exists but disagree on which sites are most ecologically diverse, and none of them explains PP-style monument scale through terrestrial-buffer ecological diversity alone.

The cross-LMV ecoregion map showing all eleven sites with 25-km foraging buffers on the EPA Level IV ecoregion polygon background is now main-text Figure 13.

**Category-level recalibration.** A reasonable concern about the L4-class Shannon analysis is that ecoregion classes are not equally distinct: an "upland" ecoregion is fundamentally different from a "lowland alluvial" ecoregion, while different lowland subtypes (Holocene Meander Belt vs Backswamp) are subtypes of the same alluvial ecology. To address this we mapped each of the 59 L4 classes in the AR-LA-MS dataset to one of 9 broad ecological categories (Alluvial Active, Backswamp, Pleistocene Terrace, Loess Upland, Tertiary Upland, Coastal Marsh, Coastal Plain Upland, Prairie, Mountain/Hills) and recomputed Shannon entropy at the category level. Results: Frenchman's Bend and Watson Brake intersect 4 broad categories at the highest category-level entropy ($\varepsilon_{cat} = 0.314$ and $0.311$), with Caney also at 4 categories ($\varepsilon_{cat} = 0.276$). Cowpen Slough, Insley, and Jaketown likewise intersect 4 categories but at lower entropy ($\varepsilon_{cat} = 0.22$, $0.22$, and $0.12$, the last dominated by a single meander-belt class). Poverty Point and Lower Jackson intersect 3 categories (Alluvial Active, Backswamp, Pleistocene Terrace) at $\varepsilon_{cat} \approx 0.24$; the coastal Pearl River pair (Claiborne and Cedarland) intersect 3 categories at $\varepsilon_{cat} \approx 0.22$; and J.W. Copes intersects 2 categories ($\varepsilon_{cat} = 0.14$). The category-level Spearman correlation between qualitative $\varepsilon$ and category-$\varepsilon$ is $\rho = 0.64$ ($p = 0.036$), and the L4-vs-category Spearman correlation is $\rho = 0.58$ ($p = 0.060$), confirming that the category-level aggregation tracks both the rubric and the L4 index moderately while smoothing out subtype distinctions. The recalibration therefore moderately strengthens the qualitative rubric's correspondence with the GIS measurement but *does not* make PP rank highest in catchment ecological diversity. PP's distinguishing feature in the framework is its confluence position integrating multiple canoe-accessible drainages with non-synchronized hydrographs, not its 25-km terrestrial catchment heterogeneity, which is what every static-diversity proxy at any granularity will continue to underestimate.
...
Table 3 reports Spearman $\rho = +0.39$ ($p = 0.24$) between the qualitative-rubric $\varepsilon$ and observed ordinal monument investment across the eleven LMV sites. To assess robustness of the ranking against plausible scoring uncertainty, we ran a Monte Carlo perturbation of the zone weights ($\pm 0.2$ uniform per weight, clipped to $[0, 1]$, 1,000 draws; `scripts/analysis/table2_weight_perturbation.py`). Under the corrected zone coding (coastal $\varepsilon = 0.40$), the ranking is only partially stable: Poverty Point ranks first in 86% of draws, but the coastal Pearl River pair (Claiborne and Cedarland) occupies the bottom two ranks in only 21% of draws (they now tie Jaketown at baseline), and the full eleven-site rank order is never preserved (0% of draws). Mean Spearman $\rho$ across perturbations is $+0.16$ with a 95% interval of $-0.19$ to $+0.49$. The PP-first result is robust against the perturbation; the coastal-last result, which was robust under the original coastal coding (0.30), is not robust once the corrected coding credits the coastal pair's Coastal Plain Upland access. The interior ordering is not stable in either coding. This pattern reinforces the §6.4 interpretation: the static rubric identifies Poverty Point's top rank robustly, ecotone diversity admits multiple candidate sites without uniquely identifying the regional center, and the interior-versus-coastal discrimination rests on smaller margins under the corrected coding than the original presentation implied.

---

## S14. Per-material exotic decomposition: extended discussion

The main text (§6.1) reports per-material model output (copper 106 ± 30, steatite 480 ± 116, galena 702 ± 188, novaculite 6,406 ± 1,923, crystal quartz 3,190 ± 898 acquisition events) against archaeological inventories (copper 155 objects, steatite 2,221 fragments, galena 740 masses and shaped objects, novaculite 101 chipped-stone items, crystal quartz 395 raw crystals). The first thing to note is that the per-material counts are not directly comparable across the model-archaeology boundary because the counting units differ. Four considerations bear on the comparison.

*Counting-unit non-comparability across materials.* Each call to the model's `acquire_exotic` routine increments the per-material counter by one, so the model "item" represents one acquisition event, equivalent to one imported *object* (one steatite bowl, one galena mass, one copper sheet or pendant). The archaeological counts use heterogeneous units. Steatite is reported as fragments (Webb 1982 describes the 2,221 figure as fragments representing several hundred vessels), and a single 30-50 cm steatite bowl breaks readily into many fragments under burial and excavation conditions. Copper is reported as objects, and galena as masses and shaped artifacts, so those two comparisons are closer to like-for-like.
...
*The far-source materials are consistent in like units.* Copper's predictive interval contains the recovered count ($z = +1.6$), galena's contains it near the center ($z = +0.2$), and steatite on a vessel basis falls inside the interval. These are the three materials whose acquisition the framework reads as band-level index signaling of long-distance network access, and they are the materials on which the model's cost-driven acquisition rule and the record agree without correction.

*The near-source overshoot is the informative failure.* The model overpredicts novaculite by roughly 63× and crystal quartz by roughly 8×. Acquisition-versus-recovery slippage (the model predicts acquisition flow; archaeology records recovered discard) can absorb modest gaps but not two orders of magnitude, and novaculite is not preservation-limited. The structural reading is that the model's per-band acquisition probability is a function of source distance and cost only: cheap nearby materials are acquired routinely in the model, while the record shows that near-source materials are rare at the type site (novaculite is \~5% of chipped stone at PP but >80% at sites near the Ouachita source; Mitch 2023:145-146). Under the framework's own signaling logic, deposition at the hub should track signal value, which rises with source distance and difficulty of acquisition; the model does not encode value-weighted retention, and this is where it fails. The failure is reported as a genuine misfit in §6.1, and adding a value-weighted retention term is identified as a model extension in §7.4 rather than applied post hoc here.

*The steatite-galena diagnostic.* The two materials sit at almost equal source distances (~900 km vs. ~800 km), so any distance-only acquisition function predicts comparable per-band rates, and the model indeed produces slightly more galena (702) than steatite (480). The archaeological record inverts this ratio: ~3× more steatite (2,221 fragments) than galena (740), or ~7× more if Webb's bulk cache of ~2,724 additional fragments is included. Part of the inversion is the fragment-versus-vessel counting unit; the remainder, if any survives a fragments-per-vessel conversion, is consistent with steatite carrying elevated material-specific value driving acquisition above the distance-decay baseline. Steatite vessels at PP appear in bulk caches (Webb 1982) and in functional contexts that imply more than incidental ornament.

The per-material breakdown is therefore best read as a partial consistency check, not a validation. The far-source, high-signal-value materials match in like units; the steatite fragment count is unit-mismatched; and the near-source materials expose a real structural limitation of the cost-driven acquisition rule that the joint Mahalanobis test in §6.1 registers as a strong rejection. We report the misfit as informative for model development rather than reconciling it.
...
The seasonal phenology figure is presented as main-text Figure 15 (§6.7). See also §S16 below.

---

## S17. Model components used in §6 and priority follow-up work

This section consolidates the seven model components developed for the empirical evaluations in §6 and, for each, identifies the follow-up work that would tighten or extend the present implementation. Three components are central to specific §6 results (regime switching for §6.3 Watson Brake; per-event labor scaling for §6.3 and §6.6; water-route catchment for §6.5). Two are sensitivity demonstrations of internal consistency (per-site $n_{agg}$ scale ratios in §6.6; regional band allocation in §6.6). One is a plausibility check (seasonally resolved aggregation timing in §6.7). One is an independently weighted falsifiable prediction (maintenance-to-new-construction ratio for §6.8 / §7.4(h)). The two structural-architecture components reported in §S5 above (a near-threshold parameter sweep and a restructured network-saturation function) are referenced by section pointer. A consolidated list of priority follow-up work appears in §S17.8.

### S17.1 Regime switching for the Watson Brake bistable closure

(Main text §6.3; *Continuous-equilibrium treatment* in main §7.4.) **Implemented in this revision (analytical-stochastic version). Central to the §6.3 closure.** A 700-year simulation samples $\sigma_{eff,t} \sim \mathcal{N}(\mu_{eff}, \sigma_{sd})$ each year at WB parameters ($\mu_{eff} = 0.319$ at $\sigma = 0.56$ and $\varepsilon = 0.43$), classifies each year above or below the analytical $\sigma^* = 0.375$, requires a minimum-persistence run length $K$ years to count as a regime transition (band-coordination delay), and accumulates monument volume during persistent above-threshold intervals at the per-event labor-scaled rate (per-event labor scaling at $\alpha = 2$; §S17.2). Sweeping $\sigma_{sd} \in [0.075, 0.20]$ and $K \in [1, 5]$ years (100 replicates per cell):

| $\sigma_{sd}$ | $K$ (yrs) | mean $f_{above}$ | predicted volume (m³) | 95% CI | overpred factor |
|---|---|---|---|---|---|
| 0.075 | 2 | 0.052 | 11,904 | [7,539, 15,733] | 0.44 |
| 0.100 | 2 | 0.081 | 18,683 | [13,111, 23,599] | 0.69 |
| 0.100 | 3 | 0.024 | 5,408 | [2,622, 8,694] | 0.20 |
| 0.125 | 3 | 0.035 | 7,942 | [4,245, 12,644] | 0.29 |
| 0.150 | 3 | 0.044 | 10,200 | [5,900, 15,577] | 0.38 |
| 0.075 | 5 | 0.001 | 157 | [0, 828] | 0.01 |
| 0.100 | 5 | 0.002 | 433 | [0, 1,639] | 0.02 |

Across the swept $(\sigma_{sd}, K)$ grid at the LiDAR-derived per-mound WB volume of 27,065 m³ (§5.1), regime-switching predictions for cumulative WB volume span ~2,800 to ~89,000 m³ at $K \in \{1, 2, 3\}$ and $\sigma_{sd} \in [0.075, 0.20]$. The observed 27,065 m³ sits inside the K=2 sweep range (~12,000 to ~35,000 m³ across this $\sigma_{sd}$ range), with K=1 overshooting (~52,000 to ~89,000 m³) and K=3 undershooting (~2,800 to ~13,600 m³). The per-event labor-scaling extension (§S17.2) closes the WB gap essentially exactly (~25,000 m³ predicted, 0.91× of observed) independently of any specific $K$ value sweep-selected against the volume observation. We therefore no longer treat the regime-switching closure as the principal reconciliation. The regime-switching extension's substantive role in the framework is to account for the episodic *tempo* of WB construction (the 200+-year inter-stage gaps), which it does at any $K \geq 1$ given the WB threshold proximity. This episodic structure is the prediction the framework makes about WB irrespective of the volume figure. The independent ethnographic anchoring of $K$ from multi-band cooperation timescales (Wiessner 2002:434-435; Kelly 2013:159-167) remains identified as priority follow-up.

Reproducible from `scripts/analysis/regime_switching_simulation.py`, with outputs in `results/sensitivity/regime_switching_wb.json`. Full ABM-level regime-switching with endogenous strategy switching and explicit annual fitness comparison remains as future work.

### S17.2 Per-event labor scaling

(Main text §6.1; *Labor-to-volume calibration* in main §7.4.) **Implemented in this revision. Central to the §6.3 Watson Brake closure and the §6.6 cross-LMV magnitude correlation.** We add a power-law scaling factor $m^3/\text{unit}(n) = b_0 \cdot (n_{agg} / 25)^\alpha$, where $b_0 = 5{,}779$ m³ per $M_g$ unit anchors PP to its observed 750,000 m³ and the exponent $\alpha$ encodes how strongly per-crew output scales with crew size. Sweeping $\alpha \in \{0, 0.5, 1, 1.25, 1.5, 1.75, 2, 2.5\}$ and comparing predicted-vs-observed volumes across the eleven LMV sites: the rank-correlation Spearman $\rho$ is invariant at $0.85$-$0.90$ (the scaling factor preserves relative ordering by design). The absolute calibration is closest at $\alpha \approx 2.0$, where mean $\log_{10}(\text{pred}/\text{obs}) = +0.10$ across the sample (all sites within ~10$^{0.5}$ = 3$\times$ of observed volume; std of log-ratios is 0.54). At $\alpha = 2.0$, the WB prediction drops from 240,000 m³ to ~25,000 m³, essentially matching the LiDAR-derived per-mound volume of 27,065 m³ (§5.1) within ~9% (ratio 0.91×). J.W. Copes drops from 90,000 m³ to ~1,300 m³ (vs observed 500 m³, ~2.6$\times$). Insley drops from 180,000 m³ to ~10,400 m³ (vs observed 14,000 m³, ~0.7$\times$, slight underpredict).
...
**Parameter sensitivity.** A sensitivity sweep across NETWORK_BONUS $\in \{0.00, 0.025, 0.05, 0.10, 0.20\}$ and $N_{thresh} \in \{2, 3, 5, 8\}$ (20 cells) confirms the winner-takes-most outcome is robust to parameter choice: 17 of 20 cells produce the same PP = 43 / Jaketown = 7 split (ρ = +0.39 against observed scale), and the remaining 3 cells (those with $N_{thresh} = 8$ that suppresses the Jaketown attractor below threshold) produce PP = 50 (winner-takes-all) with ρ = +0.52. Across all 20 cells, ρ ranges from +0.39 to +0.52 (median +0.39). The framework's endogenized-$n_{agg}$ prediction is therefore not a parameter artifact: the qualitative result (PP as dominant regional attractor, Jaketown as secondary, all others as zero) holds across the tested parameter space. Reproducible from `scripts/analysis/regional_band_allocation_sensitivity.py`, with outputs in `results/sensitivity/regional_band_allocation_sensitivity.json`.

The $\rho = +0.39$ against observed scale is the framework's genuine (n_agg-endogenized) magnitude-prediction power, substantially weaker than the $\rho = +0.91$ that emerges when literature $n_{agg}$ values are supplied as inputs (§S17.5 below). This confirms the partial-correlation finding directly: the framework's $\varepsilon$ predicts location of regional attractors but not the gradation of attained $n_{agg}$ between PP and the smaller LMV monument-building sites. Reproducible from `scripts/analysis/regional_band_allocation.py` and `regional_band_allocation_sensitivity.py`, with outputs in `results/sensitivity/regional_band_allocation.json` and `regional_band_allocation_sensitivity.json`. The full version with hydrographic routing on a digital elevation model, carrying-capacity dynamics, path-dependence over multi-century timescales, and stochastic exploration / exploitation balance remains as future work that would be expected to soften the winner-takes-most outcome and produce more graded $n_{agg}$ predictions matching the convergence-model values.

### S17.4 Covariance-based, water-route-aware travel-time catchment

(Main text §6.5; *Ecotone operationalization* in main §7.4.) **Implemented in this revision. Central to the §6.5 multi-drainage analysis.** The static-diversity proxies of main §6.4 use a 25-km circular terrestrial buffer derived from Kelly's (2013) walking day-trip radius. We replace this with a canoe-day catchment per site, identifying which independent shortfall regimes from S15 are accessible by canoe. The empirical correlation matrix from the modern USGS gauge analysis (Mississippi-Yazoo $r = 0.11$, Mississippi-Tensas $r = 0.34$, Tensas-Yazoo $r = 0.54$, Bayou Maçon-Tensas $r = 0.90$) is used to drive a 5,000-year stochastic simulation of correlated drainage productivity. The water-route $\varepsilon_{wr}$ is the variance reduction in total accessible productivity relative to a single-drainage baseline.

Per-site water-route $\varepsilon$ (`scripts/analysis/water_route_catchment_epsilon.py`):

| Site | Independent regimes | $\varepsilon_{wr}$ |
|---|---|---|
| Poverty Point | 4 ([Bayou Maçon + Tensas clustered at $r = 0.90$], Mississippi, Yazoo, Macon Ridge upland) | 0.152 |
| Lower Jackson | 4 (same four regimes, partial access to the aquatic three) | 0.142 |
| Jaketown | 3 (Mississippi, Yazoo, Upland) | 0.149 |
| Watson Brake | 2 (Ouachita-system + Upland) | 0.114 |
| Frenchman's Bend | 2 (Ouachita-tributary + Upland) | 0.114 |
| Insley | 2 ([BMacon + Tensas] partial + Upland) | 0.077 |
| Caney | 2 (Tensas-tributary partial + Upland) | 0.064 |
| J.W. Copes | 2 (Tensas + Upland partial) | 0.065 |
| Cowpen Slough | 1 (Red/Black ≈ Ouachita-system; marginal upland excluded per §6.5) | 0.006 |
| Claiborne, Cedarland | 2 (PearlGulf + Coastal Plain Upland partial) | 0.065 |

The water-route $\varepsilon$ ranking places PP, Lower Jackson, and Jaketown in the top band (these three are the only sites with substantial multi-regime canoe access), Watson Brake and Frenchman's Bend mid (Ouachita-system + upland), and the small-tributary interior sites lower. Cowpen Slough sits at the bottom of the interior ranking under the §6.5 single-regime coding ($\varepsilon_{wr} = 0.006$). The coastal pair registers $\varepsilon_{wr} = 0.065$ from the Pearl-estuarine plus Coastal Plain Upland pairing, but both regimes are structurally distinct from the LMV interior, so the value is not directly comparable to the interior sites'.

**Joint test combining §S17.2 (per-event labor scaling at $\alpha = 2$), §S17.5 (per-site $n_{agg}$), and the water-route $\varepsilon$ developed here.** Combining the three implemented extensions: predicted volumes correlate with observed monument scale at Spearman $\rho = +0.89$ (vs ordinal, $p < 0.001$) and $\rho = +0.92$ (vs observed volume, $p < 0.001$). At the canoe-day catchment, the predicted PP volume = 750,000 m³ (calibration anchor); WB = 24,585 m³ (0.91× of LiDAR-derived per-mound 27,065 m³, §5.1); Insley = 10,375 m³ (0.7$\times$ observed 14,000 m³); Lower Jackson = 49 m³ (correctly identifies it as the smallest above-threshold site at $n_{agg} = 1$). The water-route operationalization marginally improves the magnitude correlation over the static-rubric Spearman $\rho = +0.85$ to $+0.90$, but the substantial improvement comes from making the framework's central $\varepsilon$ claim *measured* rather than asserted: PP's distinguishing feature is now the integration of three to four substantively independent shortfall regimes via canoe-day catchment, supported empirically by the S15 correlation matrix.

Reproducible from `scripts/analysis/water_route_catchment_epsilon.py`, with outputs in `results/sensitivity/water_route_epsilon.json`. Caveats: the canoe-access classification is based on published site-catchment descriptions and standard-form canoe-travel speeds (~30 km/day on navigable channels). A rigorous catchment with hydrographic routing on a digital elevation model and seasonal navigability constraints would refine the access weights but is unlikely to materially change the qualitative ranking. Pre-Holocene paleo-discharge correlations may also differ from modern values. The S15 modern correlations should be read as a first-order approximation.
...
The script's broader-window output (substantial aggregation share in spring fish-spawn months and summer aquatic months) is *not* tautological in the same way: the framework's multi-zone access enables aggregation throughout the year, while published peripheral-site plant-remains interpretations emphasize fall-winter as the documented season and infer spring dispersal. This generates a genuine falsifiable prediction. Because acidic soils have dissolved bone at the type-site core, the test cannot run on existing faunal collections. It requires new research using non-faunal seasonality proxies (macrobotanical assemblages from new excavations, phytolith and pollen records from contexts that preserve those signatures, stable-isotope sequencing of preserved tooth or shell where present, and microbotanical residues). A multi-peak signal across mast, fish-spawning, and summer aquatic months across multiple such proxies would be consistent with the framework. An exclusively fall-winter signal would tension with it.

Reproducible from `scripts/analysis/seasonal_aggregation_timing.py`, with outputs in `results/sensitivity/seasonal_aggregation_timing.json`. The full version (within-month time resolution, species-resolved subsistence dynamics, non-LMV-derived phenology calendar) remains as future work that would test the framework's seasonality prediction non-circularly.

### S17.7 Maintenance-to-new-construction ratio under signal depreciation

(Main text §6.8; falsifiable prediction §7.4(h).) Derived from the geometric depreciation recurrence $M_g(t+1) = (1 - \delta) M_g(t) + I_g$, each period's investment flow decomposes into the share that offsets depreciation, $\delta M_g$ (maintenance / signal renewal), and the share $I_g - \delta M_g$ that is net new construction. Their ratio is

$$R(M_g) = \frac{\delta M_g}{I_g - \delta M_g}$$

defined on the growth regime $0 \le \delta M_g < I_g$, monotonically increasing in $M_g$, and diverging at the steady state $M_g^* = I_g/\delta$ (at saturation all investment is maintenance). Positive depreciation $\delta > 0$ follows from the multi-period Spence condition under durable signals (Bliege Bird and Smith 2005). Identifying the model's maintenance flow with archaeologically observed renovation labor (and net growth with initial construction) is an interpretive mapping, not part of the derivation.

**PP-scenario trajectory.** At PP-calibrated parameters ($\sigma = 0.64$, $n_{agg} = 25$, $\delta = 0.08$), the lambda-sigma fixed point gives $M_g^* = 129.8$ units and $I_g = 10.38$ units/yr. Depreciation timescale $1/\delta = 12.5$ yr. Starting from $M_g(0) = 0$:

| Year $t$ | $M_g(t)$ | $\delta M_g$ (maint.) | $I_g - \delta M_g$ (new) | $R(M_g)$ |
|---|---|---|---|---|
| 12 | 81 | 6.5 | 3.9 | 1.65 |
| 25 | 112 | 8.96 | 1.44 | 6.2 |
| 50 | 127 | 10.16 | 0.24 | 42 |
| 75 | 129.5 | 10.36 | 0.02 | >500 |

Over the 75-year active interval (Kidder and Grooms 2024 Bayesian-modeled active-construction window), cumulative maintenance is $\approx 5.0$ times cumulative net new construction (83% of total labor is renewal). PP therefore sits deep in the maintenance-dominated regime for most of its active interval, as predicted by the framework.

**WB-scenario trajectory.** At WB-scenario parameters ($\sigma \approx 0.56$, $n_{agg} \approx 8$, same $\delta$), $M_g^* = 41.5$ units and $I_g = 3.32$ units/yr. Under the documented episodic regime (Saunders et al. 2005: 200+ year inter-stage gaps), each construction burst begins from a partly-depreciated stock and operates closer to initial construction than maintenance. Modeled as 4 episodes $\times$ 50 yr separated by 200 yr hiatuses (illustrative simplification of the actual chronology): within-episode $R \approx 4$, but the cumulative-across-700 yr event rate is dominated by the long silences.

**Event-density operationalization.** $R(M_g)$ is not directly observable in the archaeological record because preserved evidence does not cleanly separate maintenance from new construction. The framework's operational expression is event density per unit time: a maintenance-dominated regime produces many small renewal events per active year (re-cappings, surface preparations, post-circle rebuilds), while a growth-dominated regime produces fewer initial-construction events with long gaps.

| Site | Event count (lower bound) | Active interval | Event density (yr$^{-1}$) | Source |
|---|---|---|---|---|
| Poverty Point | $\geq 100$ (64 ridge components + 16 prepared surfaces beneath Mound C + plaza post-circle build-decommission-rebuild cycles) | 75 yr | $\geq 1.33$ | Hargrave et al. 2021; Clay 2023; Ortmann and Kidder 2013; Kidder and Grooms 2024 |
| Watson Brake | 11-20 (11 mounds + connecting ridges, 200+ yr inter-stage gaps) | 700 yr | 0.016-0.029 | Saunders et al. 2005 |

PP-to-WB event-density ratio: $\approx$ 47-85$\times$. The framework predicts a ratio substantially greater than unity, given that PP held $M_g$ near saturation for the duration of its 75-year active interval while WB operated episodically with depreciation gaps that reset the system toward zero between bursts. The observed contrast matches.

**Status: independently weighted test.** The PP-vs-WB event-density contrast is a second-axis test on top of the §6.3 magnitude contrast: both predictions follow from the same lambda-sigma fixed point plus the geometric depreciation recurrence, without additional free parameters. A reversal of the contrast (WB showing higher event density than PP) or its reduction below an order of magnitude would weaken the signal-half-life mechanism. The mechanism is currently supported.

**Caveats.**
- The constant-$I_g$ approximation does not capture within-interval labor variation. Mound A's compressed $\leq$90-day construction (Ortmann and Kidder 2013) and the dense early-active-interval construction at Ridge 3 West (Kidder et al. 2021) suggest peak labor early followed by tail, which the constant-$I_g$ model does not capture. The cumulative-labor decomposition is unaffected, but the year-by-year $R(t)$ trajectory is approximate.
- The published event counts mix renewal and new-construction events without a clean separator. The framework's maintenance-vs-new split cannot be directly verified at per-event resolution from the existing archaeology.
- The WB episodic structure is approximated as four equal-spaced 50-year blocks. The published WB chronology (Saunders et al. 2005) shows actual episode timing differing from this simplification. The qualitative event-density contrast does not depend on the specific episode schedule.
- The depreciation rate $\delta = 0.08$ is set from Table S2's earthen-mound weathering-rate range $[0.04, 0.16]$ but is not derived from a per-site earthwork-decay study. The PP-vs-WB contrast is robust across this range. The precise $R$ values are not.

Reproducible from `scripts/analysis/renovation_ratio.py`, with outputs in `results/sensitivity/renovation_ratio.json`.

### S17.8 Priority follow-up work
...
7. **Per-site earthwork-decay calibration of $\delta$** and labor-resolved chronology for the $R(M_g)$ event-density prediction (§S17.7). The current PP-vs-WB contrast uses Table S2's earthen-mound weathering-rate range $\delta \in [0.04, 0.16]$ rather than a site-specific decay estimate, and the published event counts mix maintenance/renewal with new construction without a clean separator. Per-site $\delta$ from sediment-cap weathering studies and event-resolved labor estimates (per-component fill volumes, per-stage labor budgets) would convert the structural event-density contrast into a quantitative $R$-vs-$R$ comparison.

The framework's contribution sits in the structural register: the threshold-crossing necessity claim, the asymmetric-flow signature, the screening claim across LMV sites, and the event-density contrast under depreciation are all defended by the analyses in the main text. The predictive register at the magnitude scale awaits the full implementations of items 1, 3, 4, 6, and 7.
