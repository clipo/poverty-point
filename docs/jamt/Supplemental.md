...
| 6.1 (exotic) | Exotic-goods count consistency | Same simulation run as 6.1 vol | Per-material counts (different model pathway for each) | Partial test; consistent for copper (after recovery), for galena (after recovery), vessel-basis for steatite |
...
| 6.6 | Cross-site magnitude $M_g(\varepsilon,n_{agg})$ | $n_{agg}$ from convergence-model literature | $\varepsilon$ from rubric | Joint $\rho = + 0.85$ to $+ 0.91$, but partial-correlation: $\varepsilon$ contributes $\leq + 0.03$ |
