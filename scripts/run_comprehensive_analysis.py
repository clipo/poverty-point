...
from src.poverty_point.parameters import (
    default_parameters, critical_threshold,
...
def run_poverty_point_calibration(duration: int = 500, n_replicates: int = 5):
    ...
    print(f"Monument (archaeological): ~{ARCHAEOLOGICAL_MONUMENT_VOLUME_M3:,} m³")

    # Note: Model monument units are arbitrary, need scaling
    scaling_factor = ARCHAEOLOGICAL_MONUMENT_VOLUME_M3 / summary['monument_mean'] if summary['monument_mean'] > 0 else 1
    print(f"Scaling factor: {scaling_factor:.1f}")

    summary['scaling_factor'] = scaling_factor
    summary['archaeological_monument'] = ARCHAEOLOGICAL_MONUMENT_VOLUME_M3
