...
from src.poverty_point.parameters import calculate_sigma_from_shortfall
...
def create_environmental_variability_figure():
    ...
    # Compute sigma using canonical formula from parameters.py
    sigma_grid = np.zeros((len(freq_vals), len(mag_vals)))

    for i, interval in enumerate(freq_vals):
        for j, mag in enumerate(mag_vals):
            sigma_grid[i, j] = calculate_sigma_from_shortfall(interval, mag)
    ...
    # Colorbar
    cbar = plt.colorbar(im, ax=ax3, label='Raw σ', shrink=0.8, pad=0.02)
...
def main():
    ...
    # Save outputs
    output_png = OUTPUT_DIR / 'figure_13_multi_drainage.png'
    output_pdf = OUTPUT_DIR / 'figure_13_multi_drainage.pdf'
