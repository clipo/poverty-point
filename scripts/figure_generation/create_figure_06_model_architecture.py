...
Output: figures/manuscript/figure_06_model_architecture.png
"""
...
from src.poverty_point.parameters import (
    W_aggregator, W_independent, critical_threshold, default_parameters,
)
...
# Font settings (sans-serif per CLAUDE.md)
...
def draw_panel_d_fitness(ax):
    ...
    # Compute sigma* from the model
    sigma_star = critical_threshold(epsilon, n, params)

    # Generate fitness curves
    sigma = np.linspace(0, 0.8, 100)
    w_agg_vals = np.array([W_aggregator(s, epsilon, n, params) for s in sigma])
    w_ind_vals = np.array([W_independent(s, params) for s in sigma])

    # Find crossing index
    diff = w_agg_vals - w_ind_vals
    idx = np.argmin(np.abs(diff))

    # Plot
    ax.plot(sigma, w_agg_vals, color=CB_COLORS['blue'], linewidth=2.5, label='Aggregator')
    ax.plot(sigma, w_ind_vals, color=CB_COLORS['red'], linewidth=2.5, label='Independent')

    # Critical threshold
    ax.axvline(x=sigma_star, color=CB_COLORS['gray'], linestyle='--', linewidth=1.5)
    ax.text(sigma_star + 0.02, 0.95, f'$\\sigma^* \\approx {sigma_star:.2f}$', fontsize=9,
            transform=ax.get_xaxis_transform())

    # Shaded regions
    ax.fill_between(sigma[:idx], 0.2, 1.1, alpha=0.1, color=CB_COLORS['red'])
    ax.fill_between(sigma[idx:], 0.2, 1.1, alpha=0.1, color=CB_COLORS['blue'])

    # Region labels
    ax.text(sigma_star / 2, 0.50, 'Independent\nfavored', ha='center', fontsize=8,
            color=CB_COLORS['red'])
    ax.text(sigma_star + (0.8 - sigma_star) / 2, 0.50, 'Aggregation\nfavored', ha='center', fontsize=8,
    ...
    ax.set_xlim(0, 0.8)
    y_min = min(w_agg_vals.min(), w_ind_vals.min()) - 0.05
    y_max = max(w_agg_vals.max(), w_ind_vals.max()) + 0.05
...
def main():
    ...
    # Save outputs
    png_path = os.path.join(OUTPUT_DIR, 'figure_06_model_architecture.png')
    pdf_path = os.path.join(OUTPUT_DIR, 'figure_06_model_architecture.pdf')
