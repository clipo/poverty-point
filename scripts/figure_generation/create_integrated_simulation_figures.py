...
from src.poverty_point.parameters import (
    ...
    critical_threshold as compute_critical_threshold,
    default_parameters,
)

# Compute sigma* from current model parameters
...
OUT_LEGACY = PROJECT_ROOT / "figures" / "integrated"  # retained for backwards compatibility
for d in (OUT_MS, OUT_SUPP, OUT_LEGACY):
...
def create_phase_transition_figure(sigma_sweep: list) -> plt.Figure:
    ...
    Validates that the model produces the predicted transition at σ* ≈ 0.57.
    """
    ...
    # Shaded regions indicate parameter zones (below/above sigma*), not
    # predicted line position. The realized dominance shifts gradually
    # because population behavior changes more slowly than the fitness
    # ranking; over the sigma_eff range tested here it moves from
    # ~ -0.96 (almost entirely independent) toward ~ -0.23 (mixed
    # population with growing aggregator share).
    ax1 = axes[0, 0]
    ...
    ax1.axvline(SIGMA_STAR, color='red', linestyle=':', linewidth=2,
                label=f'Threshold σ* = {SIGMA_STAR:.2f}')
    ax1.fill_betweenx([-1.05, 0.5], 0, SIGMA_STAR, alpha=0.10, color='purple',
                      label='σ_eff < σ* (independent fitness > aggregator)')
    ax1.fill_betweenx([-1.05, 0.5], SIGMA_STAR, 1, alpha=0.10, color='orange',
                      label='σ_eff > σ* (aggregator fitness > independent)')
    ax1.set_xlabel('Effective Environmental Uncertainty (σ_eff)', fontsize=11)
    ax1.set_ylabel('Strategy Dominance\n(−1 = all independent, +1 = all aggregator)',
                   fontsize=10)
    ax1.set_title('A. Phase Transition in Strategy Dominance', fontsize=12)
    ax1.legend(loc='upper left', fontsize=8, framealpha=0.92)
    sigma_max = max(0.6, max(sigma_values) * 1.1)
    ...
    ax2.axvline(SIGMA_STAR, color='red', linestyle=':', linewidth=2,
                label=f'Theoretical σ* = {SIGMA_STAR:.2f}')
    ax2.axhline(25, color='gray', linestyle='--', alpha=0.5,
                label='Optimal n* = 25')
    ax2.set_xlabel('Effective Environmental Uncertainty (σ_eff)', fontsize=11)
    ...
    ax3.axvline(SIGMA_STAR, color='red', linestyle=':', linewidth=2,
                label=f'Theoretical σ* = {SIGMA_STAR:.2f}')
    ax3.set_xlabel('Effective Environmental Uncertainty (σ_eff)', fontsize=11)
    ...
    ax3.set_xlim(0, sigma_max)
    ax3.grid(True, alpha=0.3)

    # Panel D: Monument vs dominance (scatter), with reference markers
    # indicating the threshold sigma* on the colorbar and the dominance
    # range over which the population transition begins.
    ax4 = axes[1, 1]
    scatter = ax4.scatter(dominance, monument, c=sigma_values, s=100,
                          cmap='viridis', edgecolor='black', linewidth=1,
                          vmin=0.0, vmax=max(0.6, max(sigma_values)))
    ...
    cbar.ax.axhline(SIGMA_STAR, color='red', linewidth=2)
    cbar.ax.text(1.5, SIGMA_STAR, f' σ* = {SIGMA_STAR:.2f}',
                 color='red', fontsize=8, va='center', ha='left')
    ax4.grid(True, alpha=0.3)

    # Identify the data points that crossed threshold (sigma_eff > sigma*)
    # and shade the corresponding monument-investment range to mark the
    # post-threshold cluster visually.
    above_thresh = [(d, m) for d, m, s in zip(dominance, monument, sigma_values)
                    if s > SIGMA_STAR]
    if above_thresh:
        m_min = min(m for _, m in above_thresh)
        m_max = max(m for _, m in above_thresh)
        ax4.axhspan(m_min * 0.95, m_max * 1.05, alpha=0.10, color='orange',
                    label=f'Post-threshold cluster (σ_eff > {SIGMA_STAR:.2f})')
        ax4.legend(loc='lower right', fontsize=8)

    # Add annotation for phase transition. Position the label inside the
    # panel using axes-fraction coordinates so it sits cleanly away from
    # the colorbar legend on the right edge.
...
def create_phase_space_figure(phase_space: list) -> plt.Figure:
    ...
    from src.poverty_point.parameters import critical_threshold
    eps_line = np.linspace(0.1, 0.5, 50)
    sigma_stars = [critical_threshold(eps, 25, _params) for eps in eps_line]
...
def create_scenario_comparison_figure(scenarios: dict) -> plt.Figure:
    ...
    scenario_labels = {
        'low': 'Low (σ=0.32, σ_eff=0.21)',
        'poverty_point': 'Poverty Point (σ=0.64, σ_eff=0.38)',
        'high': 'High (σ=1.00, σ_eff=0.65)',
        'critical': 'Critical (σ=0.87, σ_eff=0.57)'
    }
...
def create_calibration_figure(calibration: dict, scenarios: dict) -> plt.Figure:
    ...
    archaeological = {
        'monument': ARCHAEOLOGICAL_MONUMENT_VOLUME_M3,  # m³
        'exotics': 3078,     # copper + steatite + galena
        'duration': 500,     # years
    }

    # Derive scaling factor from PP scenario: archaeological target / model output
    pp_monument = scenarios['poverty_point']['final_monument']
    scaling_factor = ARCHAEOLOGICAL_MONUMENT_VOLUME_M3 / pp_monument if pp_monument > 0 else 126.7

    # Panel A: Monument comparison
    ax1 = axes[0]
    categories = ['Model\n(scaled)', 'Archaeological']
    # Use PP scenario monument (which defines the scaling) for the primary bar,
    # and calibration variance for the error bar
    monument_model = pp_monument * scaling_factor  # = ARCHAEOLOGICAL_MONUMENT_VOLUME_M3 by construction
...
def generate_all_figures():
    ...
    fig1.savefig(OUT_MS / 'figure_07_phase_transition.png', dpi=300, bbox_inches='tight')
    fig1.savefig(OUT_MS / 'figure_07_phase_transition.pdf', bbox_inches='tight')
    plt.close(fig1)

    print("2. Phase Space Figure...")
    fig2 = create_phase_space_figure(results['phase_space'])
    fig2.savefig(OUT_MS / 'figure_08_phase_space.png', dpi=300, bbox_inches='tight')
    fig2.savefig(OUT_MS / 'figure_08_phase_space.pdf', bbox_inches='tight')
    plt.close(fig2)

    print("3. Scenario Comparison Figure...")
    fig3 = create_scenario_comparison_figure(results['scenarios'])
    fig3.savefig(OUT_LEGACY / 'fig_scenario_comparison.png', dpi=300, bbox_inches='tight')
    fig3.savefig(OUT_LEGACY / 'fig_scenario_comparison.pdf', bbox_inches='tight')
    plt.close(fig3)

    print("4. Calibration Figure...")
    fig4 = create_calibration_figure(results['calibration'], results['scenarios'])
    fig4.savefig(OUT_LEGACY / 'fig_calibration.png', dpi=300, bbox_inches='tight')
    fig4.savefig(OUT_LEGACY / 'fig_calibration.pdf', bbox_inches='tight')
    plt.close(fig4)

    print("5. Summary Figure...")
    fig5 = create_summary_figure(results)
    fig5.savefig(OUT_LEGACY / 'fig_summary.png', dpi=300, bbox_inches='tight')
    fig5.savefig(OUT_LEGACY / 'fig_summary.pdf', bbox_inches='tight')
    plt.close(fig5)

    print(f"\nAll figures saved to: figures/manuscript/, figures/supplemental/, figures/integrated/")
