...
# Import canonical sigma formula and critical threshold
from src.poverty_point.parameters import calculate_sigma_from_shortfall, critical_threshold, default_parameters

# Critical threshold (computed from model parameters)
_params = default_parameters()
SIGMA_CRITICAL = critical_threshold(0.35, 25, _params)
...
def create_panel_d_sigma(ax):
    """Panel D: Environmental uncertainty index calculation for LMV cases."""
    # Show how sigma is calculated from components for the three LMV
    # mound-building cases the manuscript discusses.
    sites = ['Poverty\nPoint', 'Watson\nBrake', 'Jaketown']
    frequencies = [10, 12, 11]
    magnitudes = [0.45, 0.40, 0.42]
...
def create_panel_e_comparison(ax):
    """Panel E: LMV site sigma comparison."""
    sites = ['Poverty\nPoint', 'Watson\nBrake', 'Jaketown', 'Lower\nJackson',
             'Frenchman\'s\nBend']
    sigmas = [0.64, 0.56, 0.58, 0.56, 0.56]
    sigma_err = [0.12, 0.10, 0.11, 0.10, 0.10]
    monuments = [True, True, True, True, True]
