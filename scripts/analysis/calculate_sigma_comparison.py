...
# Import canonical sigma formula from parameters.py (single source of truth)
from src.poverty_point.parameters import (
    calculate_sigma_from_shortfall,
    calculate_sigma_from_cv,
)


def calculate_sigma(frequency: float, magnitude: float) -> float:
    """
    Calculate environmental uncertainty parameter (sigma) from shortfall parameters.

    Delegates to the canonical formula in parameters.py:
        sigma = magnitude * sqrt(20 / frequency), clipped to [0, 1]
    ...
    -------
    float
        Environmental uncertainty parameter sigma (0 to 1)
    """
    return calculate_sigma_from_shortfall(frequency, magnitude)
...
# Model critical threshold (from numerical root-finding with exponential survival)
SIGMA_CRITICAL = 0.57
...
uncertainty above the critical threshold (sigma* ~ 0.57) favors the emergence of
costly signaling behavior, manifested archaeologically as monument construction.

Poverty Point's environmental uncertainty (sigma ~ 0.64), derived from:
